...
- 服务器提及通过官方 `matcher` 扩展点按光标识别正文中的 `@`，支持直接接在中文、英文及标点后，选中后由空白结束补全。发送时按显示名称字面值和尾部边界识别引用，避免正则转义导致正文引用遗漏。
- 引用候选与分类列表复用 `useSynchronousSelectedItemScroll`：高亮到达可视区域边缘后，在绘制前同步滚动列表内容，无平滑滚动和高亮渐变。高亮只跟随选中状态，滚轮不改变选择。通用 `CommandList` 也内置相同逻辑，终端补全、可创建下拉框及其他命令列表不再各自维护滚动代码。
