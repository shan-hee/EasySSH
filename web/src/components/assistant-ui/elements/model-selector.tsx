  ...
  "focus-visible:ring-ring/50 flex w-fit items-center justify-between gap-2 overflow-hidden rounded-md text-sm whitespace-nowrap transition-colors outline-none focus-visible:ring-1 disabled:cursor-not-allowed disabled:opacity-50 [&_svg]:pointer-events-none [&_svg]:shrink-0 [&_svg:not([class*='size-'])]:size-3.5",
  {
    variants: {
      variant: {
        ...
        ghost: "border-0 bg-transparent shadow-none hover:bg-accent hover:text-accent-foreground dark:bg-transparent dark:hover:bg-accent",
...
function ModelSelectorValue({ placeholder, showEffort = true, className }: ModelSelectorValueProps) {
...
function ModelSelectorSearch({ placeholder, ...props }: ModelSelectorSearchProps) {
  ...
  return (
    <CommandInput
      ...
      placeholder={placeholder ?? t("auiModelSearch")}
      {...props}
    />
  )
}
