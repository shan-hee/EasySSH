      ...
      {isOpen &&
        createPortal(
          <div
            ...
            role="dialog"
            aria-modal="true"
            className="aui-image-zoom-overlay fade-in animate-in fixed inset-0 z-50 flex items-center justify-center bg-background/80 duration-200"
            onClick={handleClose}
            aria-label={t("auiZoomedImage")}
          >
            <img
...
function ImageGenerating({ className }: { className?: string }) {
  ...
  return (
    ...
    >
      ...
      <span className="sr-only">{t("auiGeneratingImage")}</span>
    </div>
  )
}
...
function ImageContentFilterError({ className, reason }: { className?: string; reason?: string }) {
  ...
  return (
    ...
    >
      ...
      <p className="text-sm font-medium">{t("auiImageFailed")}</p>
    ...
    <button
      ...
      onClick={async () => {
        ...
        try {
          ...
          // The host reports generation failures through the image part status.
        } finally {
      ...
      aria-label={t("auiRegenerateImage")}
...
const ImageImpl: ImageMessagePartComponent = (props) => {
  ...
  if (status?.type === "incomplete" && status.reason === "content-filter") {
    return (
      <ImageRoot>
        <ImageContentFilterError reason={t("auiImageBlocked")} />
      </ImageRoot>
    )
  }
  ...
  return (
    <ImageRoot>
      <ImageZoom src={image} alt={filename || t("auiImageContent")}>
        <ImagePreview src={image} alt={filename || t("auiImageContent")} />
      </ImageZoom>
      <ImageFilename>{filename}</ImageFilename>
    </ImageRoot>
  )
