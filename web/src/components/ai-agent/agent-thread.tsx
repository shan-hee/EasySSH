...
function AgentTool(part: ToolCallMessagePartProps) {
  ...
  const waiting = part.approval != null && part.approval.approved === undefined && !part.approval.resolution
  const target = text(args.server_name || args.server_id || args.host) || scope?.server_name || scope?.host
  const cwd = text(args.cwd || args.working_directory)
  return (
    <ToolFallback {...part} toolName={text(metadata.displayName) || part.toolName} disabled={running}>
      {waiting && (
        <div className="space-y-1 border-l-2 border-status-warning/40 pl-3 text-xs text-muted-foreground">
          ...
          <p>{tText(metadata.dangerous === true ? "approvalImpactDangerous" : "approvalImpactRoutine")}</p>
        </div>
      )}
    </ToolFallback>
  )
...
function AgentData({ name, data }: { name: string; data: unknown }) {
  ...
  if (name === "tool-status" && typeof value.text === "string")
    return <ThinkingIndicator label={value.text} className="px-2 py-1 text-xs" />
  return null
}
