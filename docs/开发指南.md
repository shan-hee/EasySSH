...
###  一键更新指令（适用于快速启动）部署，会清理数据
```bash
docker stop easyssh && docker rm easyssh && docker pull shanheee/easyssh:latest && docker run -d --name easyssh -p 8520:8520 -p 8000:8000 shanheee/easyssh:latest && echo "EasySSH 快速更新完成！"
```

###  一键更新指令（适用于生产环境）部署，会保留数据
```bash
#!/bin/bash
...
# 停止旧容器
echo "⏹️  停止旧容器..."
docker stop easyssh-prod 2>/dev/null || true

# 删除旧容器（保留数据卷）
echo "🗑️  删除旧容器..."
docker rm easyssh-prod 2>/dev/null || true
...
# 检查服务状态
if docker ps | grep -q easyssh-prod; then
    echo "✅ 升级完成！"
    echo "🌐 访问地址: http://localhost:8520"
    echo "📋 查看日志: docker logs easyssh-prod -f"
else
    echo "❌ 升级失败，请检查日志:"
