...
```bash
docker stop easyssh && docker rm easyssh && docker pull shanheee/easyssh:latest && docker run -d --name easyssh -p 3000:3000 -p 8000:8000 shanheee/easyssh:latest && echo "EasySSH 快速更新完成！"
```
