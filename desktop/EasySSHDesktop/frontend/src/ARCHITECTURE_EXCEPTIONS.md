...
| Router/provider exports from workspace desktop entry | `shell/desktop-providers.tsx` still consumes shared provider exports from `@easyssh/ssh-workspace/desktop` | Provider wiring is still coupled to the current facade package | Move provider exports to a shell-neutral shared entry or import from their owning modules directly |
