...
import { Bell, Check, CheckCheck, CircleCheck, Info, Trash2, TriangleAlert, XCircle, toast } from "@easyssh/ssh-workspace/desktop"
...
export function DesktopNotificationCenter({ onOpenTask }: { onOpenTask: (taskID?: string) => void }) {
  ...
  const load = useCallback(async (notifyOnError = false) => {
    try {
      ...
      if (notifyOnError) {
        toast.error(t("notificationsLoadFailed"))
      }
    }
  }, [t])
  ...
  const markRead = useCallback(async (item: DesktopNotification) => {
    if (!item.read_at) {
      try {
        ...
        toast.error(t("notificationsMarkReadFailed"))
      }
    }
  ...
  const markAllRead = useCallback(async () => {
    try {
      await desktopNotificationsApi.markAllRead()
      await load()
    } catch (error) {
      console.error("Failed to mark all desktop notifications as read:", error)
      toast.error(t("notificationsMarkAllReadFailed"))
    }
  }, [load, t])

  const remove = useCallback(async (id: string) => {
    try {
      await desktopNotificationsApi.remove(id)
      await load()
    } catch (error) {
      console.error("Failed to delete desktop notification:", error)
      toast.error(t("notificationsDeleteFailed"))
    }
  }, [load, t])

  return <Popover open={open} onOpenChange={(value) => { setOpen(value); if (value) void load(true) }}>
    ...
    <PopoverContent align="end" sideOffset={8} className="w-[min(420px,calc(100vw-24px))] p-0 [--wails-draggable:no-drag]">
      <div className="flex h-12 items-center justify-between border-b px-3"><span className="text-sm font-semibold">{t("notificationsTitle")}</span><Button variant="ghost" size="icon-sm" disabled={unread === 0} onClick={() => void markAllRead()} title={t("notificationsMarkAllRead")}><CheckCheck className="size-4" /></Button></div>
      <ScrollArea className="h-[min(520px,65vh)]">
        {items.length === 0 ? <div className="px-4 py-12 text-center text-sm text-muted-foreground">{t("notificationsEmpty")}</div> : <div className="divide-y">{items.map((item) => {
          ...
          return <div key={item.id} className={cn("group flex gap-3 px-3 py-3", !item.read_at && "bg-accent/35")}>
            ...
            <Button variant="ghost" size="icon-sm" className="size-7 opacity-0 group-hover:opacity-100" onClick={() => void remove(item.id)} title={t("notificationsDelete")}><Trash2 className="size-3.5" /></Button>
          </div>
        })}</div>}
      </ScrollArea>
    </PopoverContent>
