 ...
 */
export function createStoragePlugin(options: Record<string, any> = {}) {
  const defaultOptions = {
    key: (storeId: string) => `store.${storeId}`,
  ...
  return ({ store, options: storeOptions }: { store: any; options: any }) => {
    // 检查store是否启用了持久化
    const persistConfig = storeOptions.persist;
    if (!persistConfig) return;

    // 合并配置
    const storeConfig: any = typeof persistConfig === 'boolean' ? {} : persistConfig;

    const finalConfig = {
      ...config,
      ...storeConfig,
      key: storeConfig.key || (typeof config.key === 'function' ? config.key(store.$id) : config.key)
    };
    ...
    store.$subscribe(
      (_mutation: any, state: any) => {
        persistState(store, state, finalConfig as any);
      },
 ...
 */
function restoreState(store: any, config: any) {
  try {
    // 执行恢复前钩子
    ...
    // 从存储获取数据
    const storedData = getStorageData(config.key as string, config.storageType as string);
    ...
    if (storedData) {
      ...
      if (config.paths && Array.isArray(config.paths)) {
        (config.paths as string[]).forEach((path: string) => {
 ...
 * @param {Object} config 配置对象
 */
function persistState(store: any, state: any, config: any) {
  try {
    ...
    // 保存到存储
    const success = setStorageData(config.key as string, serializedData, config.storageType as string);
 ...
 */
function getStorageData(key: string, storageType: string): string | null {
  try {
    switch (storageType) {
      case 'session':
 ...
 */
function setStorageData(key: string, data: string, storageType: string): boolean {
  try {
    switch (storageType) {
      case 'session':
