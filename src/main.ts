...
// 注册自定义指令
const directivesMap = directives as Record<string, any>;
...
ensureElementPlusReady().then(() => {
  ...
  try {
    ...
    const gracefulRemove = (el: Element) => {
      ...
      for (const m of mutations) {
        if (m.addedNodes && m.addedNodes.length) {
          m.addedNodes.forEach(n => {
            if (n instanceof Element && typeof n.matches === 'function' && n.matches(SELECTOR)) {
              touched = true;
            }
          });
        }
...
// 智能数据刷新策略（优化为按需加载模式）
async function smartDataRefresh(userStore: any) {
  try {
    // 检查网络状态
...
window.addEventListener('auth:complete-logout', async () => {
  ...
  sessionStorage.setItem('auth_complete_logout', 'true');
    ...
    // 即使出错也要跳转到登录页
    sessionStorage.setItem('auth_logout_failed', 'true');
    window.location.href = '/login';
  }
});
...
window.addEventListener('auth:remote-logout', () => {
  log.warn('检测到远程登出事件，跳转到登录页（保留记住的凭据）');
...
// 智能刷新触发器
async function triggerSmartRefresh(reason: string) {
  if (isRefreshing) {
...
// 网络恢复监听 - 网络从断开到恢复时刷新数据
window.addEventListener('online', () => {
  triggerSmartRefresh('网络恢复');
});

// 避免因多个窗口同时刷新造成的冲突（跨标签页协调）
window.addEventListener('storage', event => {
  if (event.key === 'last_data_refresh' && event.newValue) {
    const now = Date.now();
    const lastRefresh = parseInt(event.newValue);
    // 如果是其他标签页刚刷新过（5秒内），当前标签页就不再刷新
    if (now - lastRefresh < 5000) {
      log.debug('检测到其他标签页刷新数据，当前标签页跳过刷新');
      return;
    }
  }
});
