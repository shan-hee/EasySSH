...
type TabType = 'newConnection' | 'terminal' | 'sftp' | string;
type TabItem = {
  title: string;
  type: TabType;
  path: string;
...
export const useTabStore = defineStore(
  ...
  () => {
    ...
    const addNewConnection = async (): Promise<void> => {
      ...
      try {
        // 强制创建新数组以触发响应式更新
        const newTabs = [...state.tabs, newTab] as TabItem[];
        state.tabs = newTabs;
    ...
    // 添加终端标签页
    const addTerminal = async (connectionId: string): Promise<number> => {
      // 默认标题
      let tabTitle = '终端';
    ...
    // 添加SFTP浏览器标签页
    const addSftpBrowser = async (connectionId?: string): Promise<number> => {
      // 默认标题
      let tabTitle = 'SFTP 浏览器';
      let connectionInfo = null;
      const navigationPath = '/sftp';
    ...
    // 添加设置标签页 - 现在打开用户设置模态框
    const addSettings = async (): Promise<void> => {
      try {
        // 发送打开用户设置的全局事件
    ...
    const switchTab = (index: number): void => {
      ...
      // 如果标签页有关联的路径，则导航到该路径
      const tab = state.tabs[index] as TabItem;
      if (tab && tab.path) {
        // 对于终端页面特殊处理
    ...
    // 关闭标签页
    const closeTab = (indexOrPath: number | string): void => {
      let index: number = indexOrPath as number;

      // 如果传入的是路径，需要找到对应的索引
      if (typeof indexOrPath === 'string') {
        index = (state.tabs as TabItem[]).findIndex((tab: TabItem) => tab.path === (indexOrPath as string));
        if (index === -1) {
      ...
      // 得到要关闭的标签信息，用于后续处理
      const closingTab = state.tabs[index] as TabItem;
      ...
      if (
        ...
        closingTab.data &&
        closingTab.data.connectionId
      ) {
        try {
          const terminalStore = useTerminalStore();
          // 检查是否有其他标签使用同一个连接
          const sameConnectionTabs = (state.tabs as TabItem[]).filter(
          ...
          if (sameConnectionTabs.length === 0) {
            ...
            const disconnectTimeout = setTimeout(() => {
              ...
              try {
                ...
                if (
                  ...
                  terminalStore.sessions[closingTab.data.connectionId]
                ) {
                  const sessionId = (terminalStore.sessions as Record<string, any>)[closingTab.data.connectionId];
                  (sshService as any)
                    .releaseResources(sessionId)
                    .catch((error: any) => log.error(`强制释放SSH资源失败: ${error?.message}`));
                }
              } catch (error: any) {
            ...
            // 尝试正常断开连接
            Promise.resolve(terminalStore.disconnectTerminal(closingTab.data.connectionId)).then(() => {
            ...
            log.info(
              `保持终端 ${closingTab.data.connectionId} 连接，还有 ${sameConnectionTabs.length} 个标签使用它`
            );
      ...
      if (state.activeTabIndex >= index && state.activeTabIndex > 0) {
        ...
        // 确保导航到新的活动标签页对应的路由
        const newActiveTab = state.tabs[state.activeTabIndex] as TabItem;
        if (newActiveTab && newActiveTab.path) {
          // 对终端标签特殊处理，使用不带参数的路径
          if (
            newActiveTab.type === 'terminal' &&
            newActiveTab.data &&
            newActiveTab.data.connectionId
          ) {
            // 使用会话存储
            const sessionStore = useSessionStore();
      ...
      if (closingTab && closingTab.type === 'terminal' && state.tabs.length > 0) {
        // 检查是否还有其他终端标签
        const hasOtherTerminals = (state.tabs as TabItem[]).some((tab: TabItem) => tab.type === 'terminal');
        if (!hasOtherTerminals) {
          // 如果终端视图当前是活动的，导航到其他标签或首页
          if (router.currentRoute.value.path.startsWith('/terminal')) {
            const newTab = state.tabs[state.activeTabIndex] as TabItem;
            if (newTab) {
              // 检查新的活动标签是否也是终端标签
    ...
    // 保存连接
    const saveConnection = (index: number): void => {
      const tab = state.tabs[index] as TabItem;
    ...
    const resetState = (): void => {
      ...
      try {
        const terminalStore = useTerminalStore();
        // 找出所有终端标签页并断开连接
        (state.tabs as TabItem[]).forEach((tab: TabItem) => {
          if (tab.type === 'terminal' && tab.data && tab.data.connectionId) {
            Promise.resolve(terminalStore.disconnectTerminal(tab.data.connectionId as string)).catch(
              (error: any) => {
    ...
    // 更新标签页标题
    const updateTabTitle = (path: string, newTitle: string): void => {
      if (!path || !newTitle) {
      ...
      if (path === '/terminal') {
        ...
        if (activeId) {
          // 寻找匹配会话ID的终端标签
      const matchingTab = (state.tabs as TabItem[]).findIndex(
        (tab: TabItem) => tab.type === 'terminal' && tab.data && tab.data.connectionId === activeId
      );
      ...
      // 找到对应路径的标签
      const tabIndex = (state.tabs as TabItem[]).findIndex((tab: TabItem) => tab.path === path);

      if (tabIndex === -1) {
        // 如果找不到完全匹配的路径，尝试找到与路径前缀匹配的标签
        // 例如，如果path是/terminal/12345，那么也会匹配到/terminal/:id
        const partialMatch = (state.tabs as TabItem[]).findIndex((tab: TabItem) => {
    ...
    // 直接设置当前活动标签页的标题
    const setActiveTabTitle = (newTitle: string): boolean => {
    ...
    // 更新标签页
    const updateTab = (index: number, tabData: Partial<TabItem>): boolean => {
      // 检查索引是否有效
    ...
    // 根据ID更新标签页标题
    const updateTabTitleById = (tabId: string, newTitle: string): boolean => {
      if (!tabId || !newTitle) {
      ...
      // 寻找匹配的标签页
      const tabIndex = (state.tabs as TabItem[]).findIndex(
        (tab: TabItem) =>
    ...
    // 连接成功处理
    const connectionSuccess = (tabId: string, connectionInfo: any): boolean => {
      if (!tabId) return false;

      const tabIndex = (state.tabs as TabItem[]).findIndex(
        (tab: TabItem) => (tab.data && tab.data.connectionId === tabId) || tab.id === tabId
      );
    ...
    // 连接失败处理
    const connectionFailed = (tabId: string, error: any, options: any = {}): boolean => {
      if (!tabId) return false;
      ...
      try {
        ...
        if (fallbackSession) {
          ...
          if ((restoredData as Record<string, unknown>).connectionId) {
            delete (restoredData as Record<string, unknown>).connectionId;
          }
          const fallbackTab = {
            title: fallbackTitle,
            type: 'newConnection',
    ...
    // 根据连接ID查找标签页
    const findTabByConnectionId = (connectionId: string): TabItem | undefined => {
      return (state.tabs as TabItem[]).find((tab: TabItem) => tab.data && tab.data.connectionId === connectionId);
    };

    // 清理连接相关的标签页状态
    const cleanupConnectionTab = (connectionId: string): boolean => {
      const tabIndex = (state.tabs as TabItem[]).findIndex(
        (tab: TabItem) => tab.data && tab.data.connectionId === connectionId
      );
