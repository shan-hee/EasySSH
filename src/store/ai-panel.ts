...
export const useAIPanelStore = defineStore('aiPanel', () => {
  // ===== 类型定义 =====
  type MessageType = 'assistant' | 'user' | string;
  ...
  type NewAIMessage = Partial<AIMessage> & {
    type?: MessageType;
  };

  interface AIPanelPreferences {
    autoShow: boolean;
    maxMessages: number;
  ...
  interface AIPanelGlobalSettings {
    isEnabled: boolean;
  ...
  // 消息历史 - 每个终端独立
  const messageHistory = ref<Record<string, AIMessage[]>>({});

  // 面板高度 - 每个终端独立
  const panelHeights = ref<Record<string, number>>({});

  // 用户偏好设置
  const preferences = reactive<AIPanelPreferences>({
  ...
  // 全局设置
  const globalSettings = reactive<AIPanelGlobalSettings>({
  ...
  const totalMessages = computed(() => {
    return Object.values(messageHistory.value).reduce<number>((total, messages) => {
   ...
   */
  const initializeTerminal = (terminalId: string): void => {
   ...
   * @param {string} terminalId 终端ID
   */
  const showPanel = (terminalId: string): void => {
    if (!terminalId) return;
   ...
   * @param {string} terminalId 终端ID
   */
  const hidePanel = (terminalId: string): void => {
    if (!terminalId) return;
   ...
   * @param {string} terminalId 终端ID
   */
  const togglePanel = (terminalId: string): boolean | void => {
    if (!terminalId) return;
   ...
   */
  const isPanelVisible = (terminalId: string): boolean => {
    return !!panelStates.value[terminalId];
  };
   ...
   */
  const addMessage = (terminalId: string, message: NewAIMessage): AIMessage | void => {
   ...
   * @returns {Array} 消息列表
   */
  const getMessages = (terminalId: string): AIMessage[] => {
    if (!terminalId) return [] as AIMessage[];
    return messageHistory.value[terminalId] || ([] as AIMessage[]);
  };
   ...
   * @param {string} terminalId 终端ID
   */
  const clearMessages = (terminalId: string): void => {
    if (!terminalId) return;
   ...
   */
  const markAsRead = (terminalId: string, messageId: string | null = null): void => {
    if (!terminalId) return;
   ...
   */
  const setPanelHeight = (terminalId: string, height: number): void => {
   ...
   * @returns {number} 高度值
   */
  const getPanelHeight = (terminalId: string): number => {
   ...
   */
  const updatePreferences = (newPreferences: Partial<AIPanelPreferences>): void => {
  ...
  const savePreferences = (): void => {
    try {
      ...
      log.error('保存AI面板偏好设置失败', { error: (error as any)?.message });
    }
  };
  ...
  const loadPreferences = (): void => {
    try {
      const saved = settingsService.get('aiPanel.preferences', {} as Partial<AIPanelPreferences>);
      Object.assign(preferences, saved);
      log.debug('AI面板偏好设置已加载');
    } catch (error: unknown) {
      log.error('加载AI面板偏好设置失败', { error: (error as any)?.message });
    }
  };
   ...
   * 保存消息历史到本地存储
   */
  const saveMessageHistory = (): void => {
    if (!globalSettings.persistHistory) return;

    try {
      // 只保存最近的消息
      const filteredHistory: Record<string, AIMessage[]> = {};
      ...
      log.error('保存AI面板消息历史失败', { error: (error as any)?.message });
    }
  };
   ...
   * 从本地存储加载消息历史
   */
  const loadMessageHistory = (): void => {
    if (!globalSettings.persistHistory) return;

    try {
      const saved = settingsService.get('aiPanel.messageHistory', {} as Record<string, AIMessage[]>);
      messageHistory.value = saved as Record<string, AIMessage[]>;
      log.debug('AI面板消息历史已加载');
    } catch (error: unknown) {
      log.error('加载AI面板消息历史失败', { error: (error as any)?.message });
    }
  };
   ...
   * @param {string} terminalId 终端ID
   */
  const cleanupTerminal = (terminalId: string): void => {
    if (!terminalId) return;
