  ...
  () => {
    ...
    async function updateConnection(id, updatedConnection) {
      try {
        ...
        if (isLoggedIn.value) {
          try {
            const response = await apiService.put(`/connections/${id}`, {
            ...
            if (error.response && error.response.status === 404) {
              ...
              try {
                // 尝试重新创建连接
                const createResponse = await apiService.post('/connections', {
                  connection: connections.value[index]
                });
    ...
    async function syncConnectionsToServer() {
      ...
      try {
        // 检查API是否可用
        const apiCheckResponse = await apiService
    ...
    async function toggleFavorite(id) {
      ...
      if (isLoggedIn.value) {
        try {
          const response = await apiService.post('/connections/favorites', {
            favorites: favorites.value
          });
