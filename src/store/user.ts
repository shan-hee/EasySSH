...
export const useUserStore = defineStore(
  ...
  () => {
    ...
    const pinnedConnections = ref<Record<string, number>>({});

    // 统一数据加载中的共享Promise，避免多处并发重复请求
    let connectionsDataInflight: Promise<any> | null = null;
    ...
    function updatePreferences(newPrefs: any): void {
      preferences.value = { ...preferences.value, ...newPrefs };
    }

    // 保存用户凭据（记住密码）
    function saveUserCredentials(username: string, password: string): void {
      try {
    ...
    // 清除保存的用户凭据
    function clearUserCredentials(): void {
    ...
    const normalizeConnection = (connection: any, index: number = 0): any => {
    ...
    const applyConnectionsOrder = (newOrder: any[]): void => {
    ...
    // 添加新连接
    async function addConnection(connection: any): Promise<string | null> {
      try {
        // 检查是否已存在相同的连接（去重）
        ...
        if (isLoggedIn.value) {
          ...
          try {
            // 同步保存到服务器
            const response: any = await apiService.post('/connections', { connection });
    ...
    // 更新连接
    async function updateConnection(id: string, updatedConnection: any): Promise<any | null> {
      try {
        ...
        if (isLoggedIn.value) {
          try {
            const response: any = await apiService.put(`/connections/${id}`, {
              connection: connections.value[index]
            });
            ...
            if (error.response && error.response.status === 404) {
              ...
              try {
                // 尝试重新创建连接
                const createResponse: any = await apiService.post('/connections', {
                  connection: connections.value[index]
                });
    ...
    // 删除连接
    async function deleteConnection(id: string): Promise<boolean | null> {
      try {
        const index = connections.value.findIndex(conn => conn.id === id);
        if (index === -1) {
          throw new Error(`未找到ID为${id}的连接`);
        ...
        if (isLoggedIn.value) {
          try {
            const response: any = await apiService.delete(`/connections/${id}`);
    ...
    async function syncConnectionsToServer() {
      ...
      try {
        // 检查API是否可用
        const apiCheckResponse: any = await apiService
    ...
    async function loadConnectionsFromServer(forceRefresh = false) {
      ...
      try {
        ...
        // 获取用户连接列表
        const connectionsResponse: any = await apiService.get('/connections', {}, requestOptions);
        if (connectionsResponse && connectionsResponse.success) {
          const list = connectionsResponse.connections || [];
          connections.value = list.map((connection: any, index: number) => normalizeConnection(connection, index));
        ...
        // 获取用户的历史记录
        const historyResponse: any = await apiService.get('/connections/history', {}, requestOptions);
        ...
        // 获取用户的置顶信息
        const pinnedResponse: any = await apiService.get('/connections/pinned', {}, requestOptions);
    ...
    // 添加到历史记录
    async function addToHistory(connection: any): Promise<void> {
      // 构造新增的历史项（本地先行）
      const newEntry = {
        id: connection.id,
      ...
      if (isLoggedIn.value) {
        try {
          ...
          const response: any = await apiService.post('/connections/history', payload);
    ...
    async function removeHistoryEntry(entryId: string): Promise<boolean> {
      ...
      if (isLoggedIn.value) {
        try {
          const response: any = await apiService.delete(`/connections/history/entry/${entryId}`);
          if (response && response.success) {
    ...
    async function clearHistory(): Promise<boolean> {
      ...
      if (isLoggedIn.value) {
        try {
          const response: any = await apiService.delete('/connections/history');
          if (!response || !response.success) {
    ...
    // 重新排序历史连接
    function reorderHistoryConnections(newOrder: any[]): void {
    ...
    // 更新历史连接顺序（用于乐观更新）
    function updateHistoryOrder(newOrder: any[]): void {
      history.value = [...newOrder];
    }
    ...
    async function toggleFavorite(id: string): Promise<boolean> {
      ...
      if (isLoggedIn.value) {
        try {
          const response: any = await apiService.post('/connections/favorites', {
            favorites: favorites.value
          });
    ...
    async function togglePin(id: string): Promise<boolean> {
      ...
      if (isLoggedIn.value) {
        try {
          const normalizedPinned = Object.keys(pinnedConnections.value).reduce(
          ...
          const response: any = await apiService.post('/connections/pinned', {
    ...
    async function reorderConnections(newOrder: any[], options: any = {}): Promise<boolean | void> {
      ...
      if (persist && isLoggedIn.value) {
        try {
          ...
          const response: any = await apiService.post('/connections/order', {
            order: payload
          });
    ...
    // 获取置顶时间（无时间戳则返回0）
    function getPinTime(id: string): number {
    ...
    async function login(credentials: any): Promise<any> {
      try {
        ...
        // 调用后端登录API
        const response: any = await apiService.post('/users/login', {
    ...
    // 验证MFA代码
    async function verifyMfaCode(code: string, tempUserInfo: any): Promise<any> {
      try {
        ...
        if (response && response.success) {
          ...
          setUserInfo((response as any).user || tempUserInfo);
          // 通知状态管理器用户已登录
          try {
            await authStateManager.onUserLogin((response as any).user || tempUserInfo);
          } catch (error: any) {
          ...
          try {
            window.dispatchEvent(
              new CustomEvent('auth:login-success', {
                detail: { user: (response as any).user || tempUserInfo }
              })
            );
          } catch (_) {}
          return { success: true };
        ...
        return {
          success: false,
          error: error.response?.data?.message || error.message || 'MFA验证失败'
        };
      }
    }

    // 登出 - 增强版，确保完全清理所有状态
    async function logout(skipApiCall: boolean = false): Promise<void> {
    ...
    // 执行完整的状态清理（保留记住的凭据）
    async function performCompleteCleanup(): Promise<void> {
    ...
    // 更新用户资料
    async function updateProfile(userData: any): Promise<any> {
      try {
        // 调用后端更新用户资料API
    ...
    // 注销所有设备
    async function logoutAllDevices(): Promise<any> {
      try {
        // 调用后端注销所有设备API
        const response: any = await apiService.post('/users/logout-all-devices');
    ...
    async function loadConnectionsOnly() {
      ...
      try {
        ...
        // 直接请求连接数据，不检查API可用性
        const connectionsResponse: any = await apiService.get('/connections');
        if (connectionsResponse && connectionsResponse.success) {
          const list = connectionsResponse.connections || [];
          connections.value = list.map((connection: any, index: number) => normalizeConnection(connection, index));
    ...
    async function loadHistoryOnDemand() {
      ...
      try {
        ...
        const result = await retryWithBackoff(
          async () => {
            const historyResponse: any = await apiService.get('/connections/history');
            if (historyResponse && historyResponse.success) {
    ...
    async function loadFavoritesOnDemand() {
      ...
      try {
        ...
        const result = await retryWithBackoff(
          async () => {
            const favoritesResponse: any = await apiService.get('/connections/favorites');
            if (favoritesResponse && favoritesResponse.success) {
    ...
    return {
      ...
      async ensureConnectionsData(force = false) {
        ...
        const run = async () => {
          ...
          if (!force) {
            try {
          const overview: any = await apiService.get('/connections/overview');
              if (overview && overview.success) {
                if (Array.isArray(overview.connections)) {
                  connections.value = overview.connections.map((connection: any, index: number) =>
          ...
          tasks.push(
            (async () => {
              try {
                const pinnedResp: any = await apiService.get('/connections/pinned');
