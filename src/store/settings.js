...
export const useSettingsStore = defineStore('settings', () => {
  ...
  const initSettings = () => {
    try {
      ...
      // 加载页签自动恢复设置（如果存在）
      try {
        const tabAutoRestoreSettings = localStorage.getItem('tab-auto-restore')
        if (tabAutoRestoreSettings) {
          const settings = JSON.parse(tabAutoRestoreSettings)
          uiSettings.autoRestoreTabs = settings.enabled
        }
      } catch (e) {
        console.error('加载页签自动恢复设置失败:', e)
      }
  ...
  const applyTheme = () => {
    try {
      ...
      console.log(`主题已应用: ${uiSettings.theme}, 语言: ${uiSettings.language}`)
    } catch (error) {
  ...
  const saveUISettings = (settings) => {
    try {
      ...
      localStorage.setItem('tab-auto-restore', JSON.stringify({
        enabled: uiSettings.autoRestoreTabs
      }))
      ...
      if (settings.language && settings.language !== getUISettings().language) {
        console.log('检测到语言变更，需要刷新页面以完全应用')
        setTimeout(() => {
