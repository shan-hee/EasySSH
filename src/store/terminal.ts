...
export const useTerminalStore = defineStore('terminal', () => {
  ...
  const state = reactive({
    ...
    // 存储终端的FitAddon实例
    fitAddons: {} as Record<string, any>,

    // 终端背景图片状态
    useBackgroundImage: false,

    // 新增：跟踪正在创建中的会话
    creatingSessionIds: [] as string[],

    // 在state中添加用于追踪监控连接状态的Map
    monitorConnectingHosts: {} as Record<string, any>,

    // 新增：终端状态映射，跟踪每个终端的详细状态
    terminalStates: {} as Record<string, string>,

    // 新增：终端初始化锁，防止并发初始化
    terminalInitLocks: {} as Record<string, boolean>
  });
   ...
   */
  const initTerminal = async (connectionId: string, container: HTMLElement): Promise<boolean> => {
    try {
      // 参数验证
      if (!connectionId) {
        log.error('未提供连接ID');
      ...
      const localConnectionsStore = useLocalConnectionsStore() as any;
      ...
      if (sessionData) {
        ...
        // 尝试使用原始连接ID查找连接信息
        let originalConnectionId: string | null = null;

        // 根据登录状态决定从哪个store获取连接
        if (userStore.isLoggedIn) {
          connection = (connectionStore as any).getConnectionById(connectionId);
          if (!connection) {
            // 如果找不到，可能是使用了新生成的会话ID，尝试查找所有连接
            const allUserConnections = ((userStore as any).connections as any[]) || [];
            for (const conn of allUserConnections) {
              if (
                (conn as any).host === (sessionData as any)?.host &&
                (conn as any).username === (sessionData as any)?.username &&
                (conn as any).port === (sessionData as any)?.port
              ) {
          ...
          connection = (localConnectionsStore as any).getConnectionById(connectionId);
          if (!connection) {
            // 如果找不到，可能是使用了新生成的会话ID，尝试查找所有连接
            const allLocal = ((localConnectionsStore as any).getAllConnections as any[]) || [];
            for (const conn of allLocal) {
              if (
                (conn as any).host === (sessionData as any)?.host &&
                (conn as any).username === (sessionData as any)?.username &&
                (conn as any).port === (sessionData as any)?.port
      ...
      // 创建SSH会话（原有逻辑）
      try {
        const sessionId: string = await (sshService as any).createSession({
          ...connection,
        ...
        // 创建终端
        const terminal = await (sshService as any).createTerminal(sessionId, container, terminalOptions);

        // 为终端元素添加标识
        if (terminal && (terminal as any).element) {
          (terminal as any).element.setAttribute('data-terminal-id', connectionId);
        }
        ...
        try {
          const session = (sshService as any).sessions.get(sessionId);
      ...
      const expectedCancellation =
        ...
        (error && typeof error === 'object' && (error as any).expectedCancellation === true) ||
   ...
   * @private
   */
  const _getTerminalOptions = async (): Promise<any> => {
    // 等待字体加载完成
  ...
  const fixXtermHelpers = () => {
    setTimeout(() => {
      const helpers = document.querySelector('.xterm-helpers') as HTMLElement | null;
      if (helpers) {
        // 确保helpers容器正确定位且不可见
        ...
        // 处理textarea元素
        const textarea = helpers.querySelector('.xterm-helper-textarea') as HTMLElement | null;
        if (textarea) {
        ...
        // 处理字符测量元素
        const charMeasure = helpers.querySelector('.xterm-char-measure-element') as HTMLElement | null;
        if (charMeasure) {
   ...
   */
  const reattachTerminal = (connectionId: string, container: HTMLElement): boolean => {
    try {
   ...
   * @param {string} connectionId - 连接ID
   */
  const fitTerminal = (connectionId: string): void => {
    // 检查终端是否存在
  ...
  const sendCommand = (
    connectionId: string,
    command: string,
    options: { clearLine?: boolean; execute?: boolean; sendCtrlC?: boolean } = {}
  ): boolean => {
    const terminal = state.terminals[connectionId];
    if (!terminal) {
    ...
    if (sessionId) {
      // 如果找到了映射的会话ID，使用它
      session = (sshService as any).sessions.get(sessionId);
      log.debug(`通过终端ID ${connectionId} 找到SSH会话ID: ${sessionId}`);
    } else {
      // 如果没有找到映射，可能connectionId本身就是会话ID
      session = (sshService as any).sessions.get(connectionId);
      if (session) {
   ...
   * @param {string} connectionId - 连接ID
   */
  const clearTerminal = (connectionId: string): void => {
    const terminal = state.terminals[connectionId];
    if (terminal) {
      terminal.clear();
    }
   ...
   */
  const _sendTestCommand = (connectionId: string, testCommand: string = 'echo "test"'): boolean => {
  ...
  const disconnectTerminal = async (connectionId: string): Promise<boolean> => {
    try {
      ...
      if (!terminal) {
        ...
        try {
          ...
          // 简化检测逻辑，统一处理其他类型终端
            ...
            // 统一的资源释放流程
            const safeDisposeTerminal = (term: any) => {
              // 1. 尝试调用标准的清理方法
      ...
      // 如果有任何资源未释放，再次尝试清理
      if (Object.values(resourceCheck).some((v: any) => v)) {
        log.debug(`检测到未完全释放的资源，再次清理: ${connectionId}`);
        for (const key in resourceCheck as any) {
          if ((resourceCheck as any)[key]) {
            const s: any = state;
            const bucket = key === 'fitAddon' ? 'fitAddons' : `${key}s`;
            s[bucket][connectionId] = null;
            delete s[bucket][connectionId];
          }
        }
      }
   ...
   * @returns {string} - 连接状态
   */
  const getTerminalStatus = (connectionId: string): string => {
   ...
   */
  const isTerminalConnected = (connectionId: string): boolean => {
   ...
   */
  const hasTerminal = (connectionId: string): boolean => {
   ...
   */
  const getTerminal = (connectionId: string): any | null => {
   ...
   * @param {string} connectionId - 连接ID
   */
  const focusTerminal = (connectionId: string): void => {
    if (state.terminals[connectionId]) {
      try {
        const terminal = state.terminals[connectionId];
   ...
   */
  const applySettingsToAllTerminals = async (settings: any): Promise<Record<string, boolean>> => {
    if (!settings) {
    ...
    const infoVerbose = (msg: string, data?: any) => (isProd ? log.debug(msg, data) : log.info(msg, data));
  ...
  // 切换背景图像状态
  const toggleBackgroundImage = (enabled?: boolean): boolean => {
  ...
  // 检查指定ID的终端是否有会话
  const hasTerminalSession = (terminalId: string): boolean => {
    return !!state.sessions[terminalId];
  };

  // 检查指定ID的终端会话是否正在创建中
  const isSessionCreating = (terminalId: string): boolean => {
