  ...
  () => {
    ...
    const closeTab = indexOrPath => {
      ...
      if (
        ...
        try {
          const terminalStore = useTerminalStore();
          // 检查是否有其他标签使用同一个连接
          const sameConnectionTabs = state.tabs.filter(
            (tab, idx) =>
              idx !== index &&
              tab.type === 'terminal' &&
              tab.data &&
    ...
    const resetState = () => {
      ...
      try {
        ...
        state.tabs.forEach(tab => {
          if (tab.type === 'terminal' && tab.data && tab.data.connectionId) {
            terminalStore.disconnectTerminal(tab.data.connectionId).catch(error => {
    ...
    const updateTabTitle = (path, newTitle) => {
      ...
      if (path === '/terminal') {
        ...
        if (activeId) {
          // 寻找匹配会话ID的终端标签
          const matchingTab = state.tabs.findIndex(
            tab => tab.type === 'terminal' && tab.data && tab.data.connectionId === activeId
          );
    ...
    const connectionSuccess = (tabId, connectionInfo) => {
      ...
      const tabIndex = state.tabs.findIndex(
        tab => (tab.data && tab.data.connectionId === tabId) || tab.id === tabId
      );
    ...
    const findTabByConnectionId = connectionId => {
      return state.tabs.find(tab => tab.data && tab.data.connectionId === connectionId);
    };

    // 清理连接相关的标签页状态
    const cleanupConnectionTab = connectionId => {
      const tabIndex = state.tabs.findIndex(
        tab => tab.data && tab.data.connectionId === connectionId
      );
