...
export const useTabStore = defineStore('tab', () => {
  ...
  // 切换自动恢复功能
  const toggleAutoRestore = (enable) => {
    state.disableAutoRestore = !enable
    
    try {
      // 通过settings store保存设置
      const settingsStore = useSettingsStore()
      const uiSettings = settingsStore.getUISettings()
      uiSettings.autoRestoreTabs = enable
      settingsStore.saveUISettings(uiSettings)
    } catch (e) {
      console.error('通过settings store保存自动恢复设置失败:', e)
      // 如果通过settings store保存失败，则回退到直接保存到localStorage
      localStorage.setItem('tab-auto-restore', JSON.stringify({
        enabled: enable
      }))
    }
    
    return state.disableAutoRestore
  }
  
  // 设置自动恢复功能状态
  const setAutoRestore = (enable) => {
    state.disableAutoRestore = !enable
  ...
  const saveStateToStorage = () => {
    localStorage.setItem('tab-store-state', JSON.stringify({
      tabs: state.tabs,
      activeTabIndex: state.activeTabIndex,
      hasUserClearedTabs: state.hasUserClearedTabs,
      // 保存时不保存disableAutoRestore属性，确保它只影响应用启动时的行为，
      // 而不影响用户手动添加的标签被保存
    }))
  }
  ...
  const loadStateFromStorage = () => {
    try {
      // 首先尝试从settings store获取自动恢复设置
      try {
        const settingsStore = useSettingsStore()
        const uiSettings = settingsStore.getUISettings()
        // 使用settings store中的设置
        state.disableAutoRestore = !uiSettings.autoRestoreTabs
      } catch (e) {
        console.error('从settings store获取自动恢复设置失败:', e)
        
        // 如果从settings store获取失败，则尝试从本地存储获取
        const autoRestoreSettings = localStorage.getItem('tab-auto-restore')
        if (autoRestoreSettings) {
          try {
            const settings = JSON.parse(autoRestoreSettings)
            state.disableAutoRestore = !settings.enabled
          } catch (e) {
            console.error('加载自动恢复设置失败:', e)
          }
        }
      }
      
      // 如果设置了禁用自动恢复，或者通过URL参数 no-restore=true 指定不恢复
      const urlParams = new URLSearchParams(window.location.search);
      const noRestore = urlParams.get('no-restore') === 'true';
      
      if (state.disableAutoRestore || noRestore) {
        state.tabs = [];
        state.activeTabIndex = 0;
        state.hasUserClearedTabs = true;
        // 导航到登录页
      ...
      const savedState = localStorage.getItem('tab-store-state')
      if (savedState) {
        const parsed = JSON.parse(savedState)
        
        // 如果用户之前清空了所有标签，则保持标签为空
        if (parsed.hasUserClearedTabs) {
          state.tabs = []
          state.hasUserClearedTabs = true
          // 导航到登录页
          nextTick(() => router.push('/'));
