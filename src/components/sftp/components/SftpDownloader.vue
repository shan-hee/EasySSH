...
export default defineComponent({
  ...
  setup(props, { emit }) {
    ...
    const simulateDownload = downloadItem => {
      ...
      const interval = setInterval(() => {
        ...
        if (progress >= 100) {
          ...
          // 延迟移除已完成的下载项
          setTimeout(() => {
            const currentIndex = currentDownloads.value.findIndex(
    ...
    const clearCompleted = () => {
      currentDownloads.value = currentDownloads.value.filter(item => item.status !== 'completed');
    };
...
.sftp-download-progress-fill {
  height: 100%;
  background-color: var(--color-primary, #409eff); /* 与主面板下载进度条颜色保持一致 */
