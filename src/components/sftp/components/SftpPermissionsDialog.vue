<template>
  ...
  >
    <div class="permissions-content" v-if="file">
      <div class="file-info">
        <div class="file-icon">
          <svg v-if="file.isDirectory" xmlns="http://www.w3.org/2000/svg" width="24" height="24" viewBox="0 0 24 24" class="folder-icon">
            <path fill="currentColor" d="M10,4H4C2.89,4 2,4.89 2,6V18A2,2 0 0,0 4,20H20A2,2 0 0,0 22,18V8C22,6.89 21.1,6 20,6H12L10,4Z" />
          </svg>
          <svg v-else xmlns="http://www.w3.org/2000/svg" width="24" height="24" viewBox="0 0 24 24" class="file-icon">
            <path fill="currentColor" d="M13,9V3.5L18.5,9M6,2C4.89,2 4,2.89 4,4V20A2,2 0 0,0 6,22H18A2,2 0 0,0 20,20V8L14,2H6Z" />
          </svg>
        </div>
        <div class="file-details">
          <div class="file-name">{{ file.name }}</div>
          <div class="file-type">{{ file.isDirectory ? '文件夹' : '文件' }}</div>
        </div>
      </div>
          ...
          <table>
            ...
            <tbody>
              <tr>
                ...
                <td><el-checkbox v-model="permissions.owner.read" @change="updateNumericPermissions" /></td>
                <td><el-checkbox v-model="permissions.owner.write" @change="updateNumericPermissions" /></td>
                <td><el-checkbox v-model="permissions.owner.execute" @change="updateNumericPermissions" /></td>
              </tr>
              <tr>
                <td class="permission-label">群组</td>
                <td><el-checkbox v-model="permissions.group.read" @change="updateNumericPermissions" /></td>
                <td><el-checkbox v-model="permissions.group.write" @change="updateNumericPermissions" /></td>
                <td><el-checkbox v-model="permissions.group.execute" @change="updateNumericPermissions" /></td>
              </tr>
              <tr>
                <td class="permission-label">其他</td>
                <td><el-checkbox v-model="permissions.other.read" @change="updateNumericPermissions" /></td>
                <td><el-checkbox v-model="permissions.other.write" @change="updateNumericPermissions" /></td>
                <td><el-checkbox v-model="permissions.other.execute" @change="updateNumericPermissions" /></td>
              </tr>
            </tbody>
          </table>
        </div>
