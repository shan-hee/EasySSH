...
class MonitoringInstance {
  ...
  private _lastStatusHash: string | null;
  private _staticDataCache: any;
   ...
   * @private
   */
  _handleBatchMessage(batchMessage: any): void {
    if (!batchMessage.items || !Array.isArray(batchMessage.items)) {
      return;
    }

    // 逐个处理批量消息中的每个项目
    batchMessage.items.forEach((item: any) => {
      try {
        switch (item.type) {
          case 'monitoring_data':
   ...
   * @private
   */
  _handleDeltaMessage(message: any): void {
    try {
      // 重构完整数据
   ...
   * @private
   */
  _reconstructDeltaData(deltaMessage: any): any {
   ...
   * @private
   */
  _updateHistoryData(data: any): void {
    try {
      // 更新CPU历史数据
   ...
   * @private
   */
  _isValidMonitoringData(data: any): boolean {
   ...
   * @private
   */
  _handleMonitoringStatus(message: any): void {
    // 处理新的差量更新格式
   ...
   * @private
   */
  _handleMonitoringDisconnected(message: any): void {
   ...
   * @private
   */
  _emitEvent(eventName: string, detail: any): void {
...
class MonitoringService {
  private instances: Map<string, MonitoringInstance>;
  private hostConnections: Map<string, MonitoringInstance>;
  private terminalToHost: Map<string, string>;
  private hostToTerminals: Map<string, string[]>;
  private attemptedConnections: Set<string>;
   ...
   */
  getInstance(terminalId: string): MonitoringInstance | null {
    if (!terminalId) {
      return null;
    }
    ...
    return this.instances.get(terminalId) || null;
  }
   ...
   * @returns {Promise<boolean>} 连接结果
   */
  async connect(terminalId: string, host: string): Promise<boolean> {
    if (!terminalId || !host) {
      return false;
    }
    ...
    if (connected) {
      ...
      const manualClose = (instance as any)._manualDisconnect === true;
   ...
   * @param {string} terminalId - 终端ID
   */
  disconnect(terminalId: string): boolean {
    if (!terminalId) {
      return false;
    }
   ...
   */
  requestSystemStats(terminalId: string) {
    const instance = this.getInstance(terminalId);
   ...
   */
  isTerminalMonitored(terminalId: string): boolean {
    if (!terminalId) {
      return false;
    }
   ...
   * @private
   */
  _hasValidMonitoringData(instance: any): boolean {
   ...
   * @private
   */
  _addTerminalToHost(terminalId: string, host: string): void {
    ...
    const terminals = this.hostToTerminals.get(host);
    if (terminals && !terminals.includes(terminalId)) {
   ...
   * @private
   */
  _removeTerminalFromHost(terminalId: string): void {
    const host = this.terminalToHost.get(terminalId);
    if (host) {
   ...
   * @private
   */
  _getActiveTerminalId(): string | null {
    // 尝试从全局状态获取
    ...
    // 后备方案：从DOM获取
    const activeTab = document.querySelector('.terminal-tab.active') as HTMLElement | null;
    if (activeTab) {
      return (activeTab as HTMLElement).dataset.terminalId || null;
    }
   ...
   * @private
   */
  _syncActiveTerminalStatus(terminalId: string): void {
    const instance = this.getInstance(terminalId);
    if (instance) {
   ...
   * @private
   */
  _syncMonitoringStatusToTerminal(terminalId: string, host: string, masterInstance: any): void {
   ...
   */
  getHistoryData(terminalId: string) {
    const instance = this.getInstance(terminalId);
   ...
   * @param {string} terminalId - 终端ID
   */
  clearHistoryData(terminalId: string): void {
    const instance = this.getInstance(terminalId);
    if (instance) {
