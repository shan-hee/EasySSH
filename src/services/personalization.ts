...
type PersonalizationSettings = {
  ...
  localOnly: boolean;
  maxItems: number;
  topKExposures: number;
  decayDays: number;
  alpha: number;
  beta: number;
  minExposuresForBoost: number;
  clampMin: number;
  clampMax: number;
  epsilon: number;
};
...
class PersonalizationService {
  private _cache: { items: Record<string, { exposures: number; selects: number; lastSelectedAt: number; ctx: Record<string, { exposures: number; selects: number; lastSelectedAt: number }> }>; lru: string[] } | null;
  private _settings: PersonalizationSettings | null;
  private _userStore: ReturnType<typeof useUserStore> | null;
  ...
  _load(): void {
    ...
    if (!this._settings) {
      this._settings = {
        ...DEFAULT_SETTINGS,
        ...(((storageService.getItem(SETTINGS_KEY, {}) as any) || {}) as Partial<PersonalizationSettings>)
  ...
  _touchLRU(key: string): void {
    ...
    lru.push(key);
    // LRU裁剪
    while (lru.length > (this._settings?.maxItems || DEFAULT_SETTINGS.maxItems)) {
      const k = lru.shift();
      if (k && this._cache.items[k]) delete this._cache.items[k];
    }
  }

  _getItem(key: string) {
    if (!this._cache) this._cache = { items: {}, lru: [] };
  ...
  _makeKey(suggestion: any): string {
    try {
  ...
  _makeHostKey(ctx: any): string {
    const host = ctx?.host || '';
    const user = ctx?.username || '';
    return host || user ? `${user}@${host}` : '';
  }

  onShow(suggestions: any[], ctx: any = {}): void {
    try {
      this._load();
  ...
  onSelect(suggestion: any, ctx: any = {}): void {
    try {
      this._load();
      if (!this.isEnabled()) return;
  ...
  // 可扩展：实际执行记录（当前并不强制）
  onExecute(_suggestion: any, _ctx: any = {}): void {
    // 预留
  }

  // 计算提升系数（返回乘法权重）
  getBoost(suggestion: any, ctx: any = {}): number {
    try {
      this._load();
      ...
      const { alpha, beta, minExposuresForBoost, decayDays, clampMin, clampMax } = this._settings as PersonalizationSettings;
  ...
  rerank(suggestions: any[], ctx: any = {}, input: string = ''): any[] {
    try {
      this._load();
      ...
      const scored = suggestions.map((s, i) => {
        ...
        const pScore = ((s as any).finalScore != null ? (s as any).finalScore : (s as any).score || 0) * b * (1 + explore);
        return { s, i, pScore };
      });
