...
type Position = { x: number; y: number; cellHeight?: number };
type CompletionSuggestion = {
  id?: string;
  text: string;
  description?: string;
  type?: 'word' | 'commands' | 'options' | 'script';
  category?: string;
  score?: number;
};
type Callbacks = {
  onSuggestionsUpdate?: (suggestions: CompletionSuggestion[], position: Position | null, selectedIndex: number) => void;
...
class TerminalAutocompleteService {
  ...
  _shouldResetOnNextInput: boolean;
  callbacks: Callbacks;
  smartDebounce: any;
  debouncedUpdate: (...args: any[]) => void;
  debouncedUpdateHighPriority: (...args: any[]) => void;
  immediateUpdate: (...args: any[]) => void;
  config: any;
  userStore: any;
  _decoration: any;
  _marker: any;
  _anchorElement: HTMLElement | null;
  _decorationSupported: boolean | undefined;
  _lastValidPosition: Position | null;
  _pendingRaf: number | null;
  _lastEmitted: { count: number; index: number; pos: Position | null };
  _enabled?: boolean;
  constructor() {
    // 当前输入缓冲区
    this.inputBuffer = '';
  ...
  _posEquals(a: Position | null, b: Position | null): boolean {
    if (!a || !b) return false;
  ...
  _emitSuggestionsUpdate(
    ...
    try {
      ...
      // 降级：直接调用原回调，避免丢失
      try {
        const safePos: Position = position ?? { x: 0, y: 0 };
        this.callbacks.onSuggestionsUpdate?.(suggestions, safePos, selectedIndex);
      } catch (_) {
        /* no-op */
      }
    }
   ...
   */
  setCallbacks(callbacks: Partial<Callbacks>) {
   ...
   */
  processInput(data: string, terminal: any): boolean {
    try {
      // 检查服务是否启用
      if (!this.isEnabled()) {
        return false;
   ...
   * @param {string} data - 输入数据
   */
  isControlCharacter(data: string): boolean {
   ...
   * @returns {boolean} 是否已处理该输入（true表示阻止默认处理）
   */
  handleControlCharacter(data: string, terminal: any): boolean {
    const charCode = data.charCodeAt(0);
   ...
   * @param {Object} terminal - 终端实例
   */
  handleEnter(terminal: any): boolean {
    // 检查是否有选中的建议
   ...
   * @returns {boolean} 是否已处理该输入（true表示阻止默认处理）
   */
  handleAnsiSequence(data: string, _terminal: any): boolean {
   ...
   * 向下导航建议列表
   */
  navigateDown(terminal: any): void {
    if (!this.isActive || this.suggestions.length === 0) {
      return;
    }
   ...
   * 更新建议显示（包含选中状态）
   */
  updateSuggestionsDisplay(terminal: any = null): void {
    try {
      // 确保建议框仍然是激活状态
      if (!this.isActive || this.suggestions.length === 0) {
        return;
   ...
   * @param {string} data - 输入数据
   */
  updateInputBuffer(data: string): void {
    // 只处理可打印字符
   ...
   * @param {Object} terminal - 终端实例
   */
  updateSuggestions(input: string, terminal: any): void {
    try {
      // 首先检查用户是否已登录
   ...
   */
  getCombinedSuggestions(input: string, terminal: any): CompletionSuggestion[] {
    // 获取输入上下文
   ...
   */
  getInputContext(input: string, terminal: any): any {
   ...
   */
  _buildHostContext(_terminal: any): any {
    return {};
  }
   ...
   * @returns {number} 单词位置
   */
  getWordPosition(commandLine: string, currentWord: string): number {
    try {
  ...
  mergeSuggestions(
    ...
    // 预计算类型优先级
    const typePriority: Record<string, number> = { script: 3, commands: 2, word: 1 };
    ...
    for (const suggestion of suggestions) {
      ...
      if (!existing) {
        ...
        // 保留优先级更高的建议
        const currentPriority = typePriority[(suggestion.type ?? 'word') as string] || 0;
        const existingPriority = typePriority[((existing as any).type ?? 'word') as string] || 0;

        if (
          currentPriority > existingPriority ||
          (currentPriority === existingPriority && (suggestion.score ?? 0) > ((existing as any).score ?? 0))
        ) {
          const matchInfo = this.calculateMatchInfo(suggestion, inputLower);
          suggestionMap.set(text, { ...suggestion, ...matchInfo });
        }
   ...
   * @returns {Object} 匹配信息
   */
  calculateMatchInfo(suggestion: CompletionSuggestion, inputLower: string): any {
    ...
    // 根据类型调整分数
    const typeMultiplier: Record<string, number> = { script: 1.2, commands: 1.1, options: 1.05, word: 1.0 };
    const typeKey = (suggestion.type ?? 'word') as string;
    matchScore *= typeMultiplier[typeKey] || 1.0;

    return {
      matchType,
      finalScore: matchScore,
      sortKey: this.getSortKey(matchType, (suggestion.type ?? 'word') as string, matchScore)
    };
  }
   ...
   * @returns {number} 排序键
   */
  getSortKey(matchType: string, suggestionType: string, score: number): number {
    // 使用位运算优化排序键计算
    let key = 0;

    // 匹配类型权重 (高16位)
    const matchWeight: Record<string, number> = { exact: 3, prefix: 2, contains: 1 };
    key |= ((matchWeight[matchType] || 0) << 16);

    // 建议类型权重 (中8位)
    const typeWeight: Record<string, number> = { script: 3, commands: 2, options: 2, word: 1 };
   ...
   */
  optimizedSort(suggestions: CompletionSuggestion[], _input: string, _context: any): CompletionSuggestion[] {
    // 使用预计算的排序键进行快速排序
   ...
   * @returns {string} 匹配类型
   */
  getMatchType(suggestion: CompletionSuggestion, input: string): string {
   ...
   */
  getTypeOrder(matchType: string, _suggestionType: string): number {
   ...
   * @returns {number} 最终分数
   */
  calculateFinalScore(suggestion: CompletionSuggestion, input: string, context: any, matchType: string): number {
    const inputLower = input.toLowerCase().trim();
    const textLower = suggestion.text.toLowerCase();
   ...
   * @returns {number} 权重值
   */
  getContextWeight(suggestion: CompletionSuggestion, _input: string, context: any): number {
    let weight = 1.0;
    ...
    if (context.commandLine) {
      const commandLine = (context.commandLine as string).toLowerCase();
      ...
      // 网络命令上下文
      if (
        (this.config.contextDetection.commandPrefixes as string[]).some((prefix: string) => commandLine.includes(prefix)) &&
   ...
   * @param {Object} terminal - 终端实例
   */
  calculatePosition(terminal: any): Position {
    try {
   ...
   * @param {Object} terminal - 终端实例
   */
  selectSuggestion(suggestion: CompletionSuggestion, terminal: any): void {
    try {
   ...
   * @param {Object} terminal - 终端实例
   */
  handleScriptSelection(suggestion: CompletionSuggestion, terminal: any): void {
    try {
   ...
   * @param {Object} terminal - 终端实例
   */
  handleWordCompletion(suggestion: CompletionSuggestion, terminal: any): void {
    try {
      // 获取当前正在输入的单词
      const currentWord = this.getCurrentWord();
      const currentInput = this.getCurrentLineInput();
  ...
  destroy() {
    ...
    // 清理AI相关资源（可选）
    try {
      if ((this as any).cleanupAI) {
        (this as any).cleanupAI();
      }
    } catch (_) {
      /* no-op */
    }
   ...
   * 确保创建并维护基于装饰器的锚点
   */
  _ensureDecoration(terminal: any): void {
    try {
      ...
      try {
        ...
        if (decoration) {
          ...
          if (typeof decoration.onRender === 'function') {
            ...
            decoration.onRender((el: HTMLElement | null) => {
              try {
                if (!el) return;
   ...
   * 获取锚点位置
   */
  _getAnchorPosition(_terminal: any): Position | null {
    try {
