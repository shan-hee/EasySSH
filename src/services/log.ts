...
class LogService {
  ...
  logIndex: number;
  overwrites: number;
  CONFIG_KEYS: { LOG_LEVEL: string; ENABLE_CONSOLE: string; COMPACT_DOM: string };
  compactDom: boolean;
  globalErrorHandlersRegistered: boolean;
  storageSyncRegistered: boolean;
  constructor() {
  ...
  _exposeGlobalExport() {
    try {
      const g: any = typeof window !== 'undefined' ? window : typeof globalThis !== 'undefined' ? (globalThis as any) : null;
      if (!g) return;
      g.EasySSH = g.EasySSH || {};
    ...
    try {
      ...
      g.addEventListener('error', (e: any) => {
        try {
          ...
          const src = e?.filename || (e?.target && (((e.target as any).src) || (e.target as any).href)) || '';
          const info = {
            type: 'UncaughtError',
            source: src,
            line: e?.lineno,
      ...
      g.addEventListener('unhandledrejection', (e: PromiseRejectionEvent) => {
        try {
  ...
  _registerStorageSync() {
    ...
    try {
      ...
      g.addEventListener('storage', (e: StorageEvent) => {
        try {
   ...
   * @private
   */
  _isValidLogLevel(level: string): boolean {
    return typeof level === 'string' && Object.prototype.hasOwnProperty.call(this.logLevels, level);
  }
  ...
  _getStorageItem(key: string): string | null {
    try {
      ...
      this.warn('无法访问localStorage', { key, error: (error as any)?.message });
      return null;
    }
  }
  ...
  setLogLevel(level: string) {
    ...
    try {
      ...
      this.warn('无法保存日志级别到localStorage', { level, error: (error as any)?.message });
    }
  }
  ...
  setEnableConsole(enable: boolean) {
    ...
    try {
      ...
      this.warn('无法保存控制台设置到localStorage', { enable, error: (error as any)?.message });
    }
  }
   ...
   * @param {any} data
   */
  infoVerbose(message: string, data?: any) {
    try {
   ...
   * @returns {Array} - 日志数组
   */
  getLogs(options: { level?: string; limit?: number; since?: Date } = {}) {
    const { level, limit, since } = options;

    // 获取有效日志（过滤掉undefined项）
    let validLogs = this.logs.filter((log: any) => log !== undefined) as any[];

    // 按级别过滤
    if (level && this._isValidLogLevel(level)) {
      const minLevel = this.logLevels[level];
      validLogs = validLogs.filter((log: any) => this.logLevels[log.level] >= minLevel);
    }

    // 按时间过滤
    if (since instanceof Date) {
      validLogs = validLogs.filter((log: any) => new Date(log.timestamp) >= (since as Date));
    }
    ...
    return validLogs.map((log: any) => ({ ...log })); // 返回深拷贝
  }
   ...
   */
  getLogStats() {
    const validLogs = this.logs.filter((log: any) => log !== undefined) as any[];
    const stats: { total: number; byLevel: Record<string, number>; errorCounts: Record<string, number>; oldestLog: Date | null; newestLog: Date | null; overwrites: number; bufferSize: number } = {
      total: validLogs.length,
      byLevel: {},
      errorCounts: Object.fromEntries(this.errorCounts as any),
      oldestLog: null,
      newestLog: null,
    ...
    // 按级别统计
    Object.keys(this.logLevels).forEach((level: string) => {
      stats.byLevel[level] = validLogs.filter((log: any) => log.level === level).length;
    });

    // 时间范围
    if (validLogs.length > 0) {
      const ts = validLogs.map((log: any) => new Date(log.timestamp).getTime());
      stats.oldestLog = new Date(Math.min(...ts));
      stats.newestLog = new Date(Math.max(...ts));
    }
   ...
   * @private
   */
  _log(level: 'debug'|'info'|'warn'|'error', message: string, data?: any) {
    // 确保已初始化
    if (!this.isInitialized) {
      this.init();
    }
   ...
   * @private
   */
  _outputToConsole(level: 'debug'|'info'|'warn'|'error', timestamp: string, message: string, data?: any) {
   ...
   * @private
   */
  _formatLocalTime(date: Date): string {
    try {
      const pad = (n: number | string, w: number = 2) => String(n).padStart(w, '0');
   ...
   * @private
   */
  _redactSensitiveInString(str: string): string {
    try {
      let s = String(str);
      // Bearer Token
   ...
   * @private
   */
  _truncateSensitive(value: any, maxLength = 20) {
   ...
   * @private
   */
  _summarizeDomNode(el: HTMLElement): Record<string, any> {
    try {
   ...
   * 压缩 HTML 字符串为摘要输出
   */
  _summarizeHtmlString(str: string, maxLen: number) {
   ...
   * @private
   */
  _simplifyObject(obj: any, depth: number = 0, maxDepth: number = 2): any {
    // 首先处理敏感信息
    ...
    if (Array.isArray(sanitized)) {
      ...
      return sanitized.map((item: any) => this._simplifyObject(item, depth + 1, maxDepth));
    }
   ...
   */
  setIgnoreError(errorPattern: string, ignore: boolean) {
   ...
   */
  exportLogs(options: { level?: string; limit?: number; since?: Date } = {}) {
    ...
    if (stats && (stats as any).oldestLog) (stats as any).oldestLog = this._formatLocalTime(new Date((stats as any).oldestLog));
    if (stats && (stats as any).newestLog) (stats as any).newestLog = this._formatLocalTime(new Date((stats as any).newestLog));
   ...
   */
  batchLog(logEntries: Array<{ level: 'debug'|'info'|'warn'|'error'; message: string; data?: any }>) {
