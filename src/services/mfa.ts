...
class MfaService {
   ...
   */
  generateSecretKey(length: number = 16): string {
    let key = '';
   ...
   */
  generateOtpAuthUri(secret: string, username: string = 'user@easyssh.com', issuer: string = 'EasySSH'): string {
   ...
   */
  generateQrCodeUrl(otpAuthUri: string, size: number = 200): string {
   ...
   */
  generateTOTP(secret: string): string | null {
    try {
      if (!secret) {
   ...
   */
  verifyTOTP(userCode: string, secret: string, window: number = 5): boolean {
    if (!userCode || !secret) {
      log.warn('验证码或密钥为空');
      return false;
    }
   ...
   */
  async enableMfa(secret: string, code: string): Promise<{ success: boolean; message: string; data?: any }> {
    try {
      // 只做格式校验
      ...
      // 直接提交API，由后端校验
      const response: any = await apiService.put<any, any>('/users/me', {
        profile: {
          mfaEnabled: true,
          mfaSecret: secret
        },
   ...
   */
  async disableMfa(code: string): Promise<{ success: boolean; message: string; data?: any; user?: any }> {
    try {
      // 只做格式校验
      if (!code || code.length !== 6 || !/^\d{6}$/.test(code)) {
        return {
      ...
      // 先调用验证API确认身份
      try {
        const verifyResponse: any = await apiService.post<any, any>('/users/verify-mfa', {
          mfaCode: code,
   ...
   */
  async verifyMfa(code: string, _userId: string): Promise<{ success: boolean; message: string; data?: any }> {
    try {
      // 使用全局存储的auth_token，不再依赖currentUser
      const token = localStorage.getItem('auth_token');
      if (!token) {
      ...
      // 调用验证MFA的API
      const response: any = await apiService.post<any, any>('/users/verify-mfa', {
        mfaCode: code,
        isMfaVerification: true
      });
   ...
   */
  _decodeBase32ToBytes(base32: string): Uint8Array | null {
    try {
      // 标准Base32字符集
   ...
   */
  _hmacSha1(key: Uint8Array, message: Uint8Array): Uint8Array | null {
    try {
      // 转换为CryptoJS支持的格式
