 ...
 * 通知服务模块，提供应用内和系统通知功能
 */
type NotificationEventType = 'internal' | 'system' | 'all';
type NotificationKind = 'success' | 'warning' | 'info' | 'error' | 'system';

type InternalNotifyOptions = {
  title?: string;
  message?: string;
  type?: Exclude<NotificationKind, 'system'>;
  duration?: number;
  showClose?: boolean;
  onClick?: () => void;
  onClose?: () => void;
...
type NotificationListener = { event: NotificationEventType; callback: (payload: any) => void };
...
class NotificationService {
   ...
   */
  notify(options: InternalNotifyOptions): NotificationHandle {
    // 默认参数
    const defaultOptions: InternalNotifyOptions = {
      title: 'EasySSH',
      message: '',
      type: 'info',
      duration: 4500,
    ...
    // 合并选项
    const notifyOptions: InternalNotifyOptions = { ...defaultOptions, ...options };

    // 保存通知历史记录
    this._addToHistory({
      title: notifyOptions.title || 'EasySSH',
      message: notifyOptions.message || '',
      type: (notifyOptions.type || 'info') as NotificationKind,
      timestamp: new Date()
    });
   ...
   */
  sendSystemNotification(options: SystemNotifyOptions): any {
   ...
   */
  success(message: string | InternalNotifyOptions, title: string = '成功', options: InternalNotifyOptions = {}): NotificationHandle {
   ...
   * @param {Object} options - 其他选项
   */
  error(message: string | InternalNotifyOptions, title: string = '错误', options: InternalNotifyOptions = {}): NotificationHandle {
    if (typeof message === 'object') {
      options = message;
    } else {
      options.message = message;
   ...
   * @param {Object} options - 其他选项
   */
  warning(message: string | InternalNotifyOptions, title: string = '警告', options: InternalNotifyOptions = {}): NotificationHandle {
    if (typeof message === 'object') {
      options = message;
    } else {
      options.message = message;
   ...
   * @param {Object} options - 其他选项
   */
  info(message: string | InternalNotifyOptions, title: string = '信息', options: InternalNotifyOptions = {}): NotificationHandle {
    if (typeof message === 'object') {
      options = message;
    } else {
      options.message = message;
   ...
   */
  addListener(event: NotificationEventType, callback: (payload: any) => void): void {
   ...
   * @param {Function} callback - 回调函数
   */
  removeListener(event: NotificationEventType, callback: (payload: any) => void): void {
   ...
   * @private
   */
  _handleNotificationClick(options: InternalNotifyOptions): void {
    if (typeof options.onClick === 'function') {
      options.onClick();
    }
  }
   ...
   * @private
   */
  _addToHistory(notification: NotificationHistoryItem): void {
    // 添加到历史记录前面
   ...
   * @private
   */
  _emitNotificationEvent(type: NotificationEventType, data: any): void {
    // 触发特定类型监听器
    this.listeners
