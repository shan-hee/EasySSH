...
class TerminalService {
  ...
  public xtermModules: any;
  public themes: Record<string, any>;
  public defaultOptions: any;
  public activeTerminalId: Ref<string | null>;
  public eventListeners: Record<string, Array<(data: any) => void>>;
  ...
  async _ensureXtermLoaded(): Promise<any> {
   ...
   */
  initTerminal(sshSessionId: string, host: string): void {
    // 检查监控服务是否已连接到目标主机
   ...
   */
  async createTerminal(id: string, container: HTMLElement, options: any = {}): Promise<any> {
    if (!id || !container) {
    ...
    try {
      ...
      if (!Object.prototype.hasOwnProperty.call(termOptions, 'logLevel')) {
      ...
      // 创建xterm实例
      const terminal: any = new (Terminal as any)(termOptions);
      ...
      try {
        ...
        if (tryWebGL && this._checkWebGLSupport()) {
          try {
            ...
            const webglAddon = new (WebglCtor as any)();
          ...
          const werr: any = webglErr as any;
        ...
        if (renderingMode !== 'webgl' && tryCanvas && this._checkCanvasSupport()) {
          try {
            ...
            const canvasAddon = new (CanvasCtor as any)();
          ...
          const cerr: any = canvasErr as any;
      ...
      Promise.race([Promise.allSettled([targetLoad, boldLoad]), (document as any).fonts.ready])
            .then(() => {
              try {
   ...
   * @private
   */
  _setupTerminalEvents(terminal: any, id: string): void {
    try {
      ...
      const setupKeyEvent = () => {
        const keyHandler = ({ key, domEvent }: any) => {
          try {
      ...
      const setupScrollEvent = () => {
        const scrollHandler = (scrollPosition: number) => {
          try {
   ...
   * @private
   */
  _emitEvent(eventName: string, data: any): void {
   ...
   */
  getTerminal(id: string): any | null {
  ...
  destroyTerminal(id: string): void {
    ...
    try {
      ...
      if (term.terminal && term.terminal._eventListeners) {
        ...
        term.terminal._eventListeners.forEach((entry: any) => {
          const { type, element, handler, listener } = entry as {
            type: string;
            element?: HTMLElement;
            handler?: any;
          ...
          try {
            ...
            log.debug(`清理终端 ${id} 事件监听器 ${type} 失败:`, msg);
          }
        });
      ...
      if (term.terminal && term.terminal.write && term.terminal._core) {
        try {
          // 尝试恢复原始的write方法
          const protoWrite = (term.terminal as any)?.constructor?.prototype?.write;
          if (protoWrite && typeof protoWrite === 'function') {
            term.terminal.write = protoWrite.bind(term.terminal);
          }
        } catch (_e) {
          // 忽略方法恢复错误
        }
        ...
        try {
          ...
          const safeDestroyTerminalAndAddons = () => {
            ...
            if (term.addons) {
              ...
              // 创建插件列表副本
              const addonEntries = (Object.entries(term.addons).filter(([_, addon]) => !!addon) as Array<[
                string,
                any
              ]>);
              ...
              if (webglEntryIndex >= 0) {
                ...
                try {
                  ...
                  const msg = e && typeof e === 'object' && 'message' in (e as any)
                    ? (e as any).message
                    : String(e);
                  log.debug('释放 WebGL 渲染器失败，已忽略:', msg);
                }
              }

              // 逐个尝试销毁其余插件
              for (const [name, addon] of addonEntries as Array<[string, any]>) {
                try {
                  if (addon && typeof addon === 'object' && typeof addon.dispose === 'function') {
                    ...
                    addon.dispose = function () {
                      try {
                        ...
                        if (msg && msg.includes('addon that has not been loaded')) {
                          log.debug(`忽略插件 ${name} 未加载错误`);
                        }
                        // 特别处理WebGL缓冲区访问错误
                        else if (
                          msg &&
                          msg.includes("Cannot read properties of null (reading 'buffers')")
                        ) {
                          log.debug(`忽略插件 ${name} WebGL缓冲区访问错误（已销毁）`);
                        }
                        // 特别处理其他WebGL相关错误
                        else if (
                          msg && (msg.includes('WebGL') || msg.includes('buffers') || msg.includes('gl context'))
                        ) {
                          log.debug(`忽略插件 ${name} WebGL相关错误: ${msg}`);
                        } else {
                          log.warn(`终端 ${id} 销毁插件 ${name} 时出错: ${msg}`);
                        }
                      }
                    };
                    addon.dispose();
            ...
            try {
              if (term.terminal._core) {
                ...
                if (typeof term.terminal._core._addonManager.registerAddon === 'function') {
                      term.terminal._core._addonManager.registerAddon = function (addon: any) {
                    ...
                    for (const addon of registeredAddons) {
                      try {
                          ...
                          addon.dispose = function () {
                            try {
                              ...
                              // 特别处理其他WebGL相关错误
                                ...
                                log.debug(`忽略核心插件WebGL相关错误: ${msg}`);
                              } else {
                                log.warn(`安全忽略插件销毁错误: ${msg}`);
                              }
                            }
                          };
                          addon.dispose();
                        ...
                        const msg = addonError instanceof Error ? addonError.message : String(addonError);
                        log.warn(`安全处理核心插件时出错: ${msg}`);
                      }
                    }
                  } else {
                ...
                try {
                  ...
                  const msg = eventsError instanceof Error ? eventsError.message : String(eventsError);
                  log.debug(`清理事件监听器失败: ${msg}`);
                }
              }
            } catch (coreManagerError: unknown) {
              const msg = coreManagerError instanceof Error ? coreManagerError.message : String(coreManagerError);
              log.debug(`清理核心插件管理器失败: ${msg}`);
            }
            ...
            try {
              ...
              if (term.terminal.element) {
                ...
                focusEvents.forEach((event: string) => {
                  try {
              ...
              const msg = disposeError instanceof Error ? disposeError.message : String(disposeError);
              log.debug(`终端实例销毁失败: ${msg}`);
            }
          };
   ...
   * @private
   */
  _applyRendererSpecificFontOptimizations(terminal: any, termOptions: any, id: string): void {
    try {
   ...
   * @private
   */
  _optimizeCanvasFontRendering(terminal: any, id: string): void {
    try {
      // Canvas渲染器特定优化
      if (terminal.setOption) {
        // 保持字体渲染一致性
   ...
   * @private
   */
  _applyCommonFontOptimizations(terminal: any, id: string): void {
    try {
      // 确保字体渲染的一致性
      if (terminal.element) {
   ...
   */
  search(id: string, query: string, options: any = {}): boolean {
    const term = this.getTerminal(id);
   ...
   * @returns {boolean} - 是否找到匹配
   */
  findPrevious(id: string): boolean {
    const term = this.getTerminal(id);
    if (!term || !term.addons || !term.addons.search) return false;
   ...
   * @private
   */
  _loadAddonSafely(terminal: any, addon: any, addonName: string, terminalId: string): boolean {
    try {
      // 检查插件兼容性
   ...
   * @private
   */
  _checkAddonCompatibility(addon: any, addonName: string, terminal: any): { compatible: boolean; issues: string[] } {
    const result: { compatible: boolean; issues: string[] } = {
      compatible: true,
      issues: []
    };
    ...
    try {
      ...
      result.issues.push(`兼容性检查异常: ${msg}`);
    }
   ...
   * @private
   */
  _checkRendererAddonCompatibility(addon: any, addonName: string, terminal: any): boolean {
    try {
      // 记录插件和终端信息用于调试
   ...
   * @private
   */
  _verifyAddonLoaded(terminal: any, addon: any, addonName: string): { success: boolean; error?: string } {
    try {
      // 检查插件是否在终端的插件管理器中注册
