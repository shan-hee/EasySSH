...
class StorageAdapter {
  private userStore: ReturnType<typeof useUserStore> | null;
   ...
   */
  async get(category: string, defaultValue: any = null): Promise<any> {
    try {
      if (this.isLoggedIn()) {
        // 登录状态：只有当设置服务已从服务器成功加载该分类时，才使用聚合结果
        try {
          if ((settingsService as any)?.isInitialized && 
              (settingsService as any)?.hasServerSettings && 
              (settingsService as any)?.isCategoryLoaded?.(category)) {
            const aggregated = (settingsService as any).settings?.[category];
   ...
   */
  async set(category: string, data: any): Promise<boolean> {
    try {
      // UI设置保持本地化，不同步到服务器
      if (category === 'ui') {
      ...
      if (this.isLoggedIn()) {
        // 登录状态：保存到服务器
        const response = await apiService.put<any, any>('/users/settings', {
          category,
          data,
        ...
        if (response.success) {
          ...
          try {
            if (category !== 'ui' && (settingsService as any).isInitialized) {
   ...
   * @returns {Promise<boolean>} 是否成功
   */
  async remove(category: string): Promise<boolean> {
    try {
      if (this.isLoggedIn()) {
        // 登录状态：从服务器删除
        const response = await apiService.delete<any>(`/users/settings/${category}`);
