...
class WordCompletionService {
  wordLibrary: Record<string, string[]>;
  config: any;
  commandOptions: Map<string, string[]>;
  phraseLibrary: PhraseItem[];
  constructor() {
    // 常用单词库
    this.wordLibrary = {
      // 基础命令（按字母顺序排列）
    ...
    // 配置
    this.config = autocompleteConfig as any;
    ...
    try {
      const linuxData = linuxCommands as Array<{ name?: string; options?: string[] }>;
      if (Array.isArray(linuxData)) {
        const names = new Set(this.wordLibrary.commands);
        linuxData.forEach(cmd => {
   ...
   */
  getWordSuggestions(input: string, limit: number = 8, context: Context = {}): Suggestion[] {
    ...
    try {
      ...
      const phraseSuggs = this.getPhraseSuggestions(
        input.trim(),
   ...
   * @returns {Array} 补全建议列表
   */
  computeWordSuggestions(input: string, limit: number, context: Context): Suggestion[] {
    ...
    for (const category of searchOrder) {
      ...
      const categoryWords = (this.wordLibrary[category] || []) as string[];
   ...
   */
  getPhraseSuggestions(input: string, limit: number = 4, context: Context = {}): Suggestion[] {
    if (!limit) return [];
    ...
    try {
      ...
      const persisted: PhraseItem[] = (() => {
        try {
          return (phraseStore as any).getAll();
        } catch (_) {
          return [];
        }
      })();
      const baseCandidates: PhraseItem[] = [...persisted, ...this.phraseLibrary];
      const candidates: PhraseItem[] = [];
      ...
      for (const item of candidates) {
        ...
        if (score > 0) {
          suggestions.push({
            ...
            type: 'commands' as const, // 在排序中优先于普通 word
            category: 'commands',
            score
          });
        }
      ...
      const learnedMeta = new Map<string, { uses: number; last: number }>(
        (persisted || []).map((p: PhraseItem) => [p.text, { uses: p.uses || 0, last: p.lastUsedAt || 0 }])
      );
      suggestions.sort((a, b) => {
   ...
   */
  determineSearchOrder(input: string, context: Context): Category[] {
    ...
    return searchOrder as Category[];
  }
   ...
   * @returns {Array} 匹配的单词
   */
  findMatches(words: string[], input: string, limit: number): string[] {
   ...
   * @returns {number} 分数
   */
  calculateWordScore(word: string, input: string, category: Category, context: Context): number {
    let score = 0;
   ...
   * @returns {string} 描述
   */
  getWordDescription(_word: string, category: Category): string {
   ...
   * @returns {boolean}
   */
  isInGitContext(commandLine: string): boolean {
   ...
   * @returns {boolean}
   */
  isInDockerContext(commandLine: string): boolean {
