...
class AuthService {
  ...
  async checkAuthStatus(): Promise<boolean> {
    try {
      ...
      try {
        ...
        const response: any = await apiService.get('/users/me', {}, { hideErrorMessage: true });
  ...
  async login(
    ...
    try {
      ...
      const response: any = await apiService.post('/users/login', { username, password });
   ...
   */
  async logout(callApi: boolean = true): Promise<boolean> {
    try {
      // 如果需要通知服务器
      if (callApi && this.token) {
        try {
          await apiService.post('/users/logout', {}, { hideErrorMessage: true } as any);
        } catch (error: unknown) {
  ...
  async register(userData: any): Promise<{ success: boolean; message: string }> {
    try {
      const response: any = await apiService.post('/users/register', userData);
  ...
  async changePassword(oldPassword: string, newPassword: string): Promise<{ success: boolean; message: string }> {
    try {
      ...
      const response: any = await apiService.post('/users/change-password', {
        oldPassword,
        newPassword
      });
  ...
  async loadUserInfo(): Promise<any> {
    try {
      ...
      // 请求用户信息
      const response: any = await apiService.get('/users/me', {}, { hideErrorMessage: true } as any);
  ...
  async refreshToken(): Promise<any> {
    ...
    // 创建新的刷新请求
    this.refreshPromise = apiService
  ...
  // 移除重复的 isAuthenticated() 方法，使用 isLoggedIn() 代替
   ...
   */
  addAuthListener(event: string, callback: (data?: any) => void): void {
   ...
   * @param {Function} callback - 回调函数
   */
  removeAuthListener(event: string, callback: (data?: any) => void): void {
