...
class SSHService {
  constructor() {
    ...
    this.keepAliveInterval = (wsServerConfig.keepAliveInterval || 30) * 1000 // 保活间隔，默认30秒
    ...
    const interval = setInterval(() => {
      ...
      if (session.socket && session.socket.readyState === WebSocket.OPEN) {
        try {
          ...
          log.debug(`发送保活消息(ping): ${sessionId}`);
        } catch (error) {
          log.warn(`发送保活消息失败: ${sessionId}`, error);
        }
      }
    }, this.keepAliveInterval / 2); // 保活检查间隔为保活间隔的一半
  ...
  _setupSocketEvents(socket, sessionId, connection, connectionState) {
    return new Promise((resolve, reject) => {
      ...
      socket.onmessage = (event) => {
        try {
          // 记录原始消息数据用于调试
          log.debug(`收到WebSocket消息(原始): ${event.data.substring(0, 200)}${event.data.length > 200 ? '...' : ''}`);
  ...
  createTerminal(sessionId, container, options = {}) {
    ...
    const terminal = new Terminal({
      ...
      fontFamily: options.fontFamily || settings.terminalFontFamily || 'monospace',
      theme: options.theme,
      cursorBlink: true,
      scrollback: 3000,
    })
...
class SFTPService {
  ...
  handleSftpMessage(message) {
    if (!message || !message.data || !message.data.operationId) {
