...
class SettingsService {
  ...
  getTerminalTheme(themeName = 'vscode') {
    ...
    if (themeName === 'custom') {
      try {
        const customThemeStr = localStorage.getItem('easyssh_custom_theme')
        if (customThemeStr) {
          const customTheme = JSON.parse(customThemeStr)
          return customTheme
        }
      } catch (error) {
        console.error('读取自定义主题失败，使用默认主题', error)
      }
    }
  ...
  getTerminalOptions() {
    ...
    return {
      ...
      fontFamily: settings.fontFamily || 'Menlo, Monaco, Consolas, monospace',
      theme: theme,
