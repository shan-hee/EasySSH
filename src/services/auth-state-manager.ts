...
class AuthStateManager {
  ...
  private listeners: Set<(event: 'login' | 'logout', data?: any) => void>;
   ...
   */
  async onUserLogin(user: any): Promise<void> {
    try {
  ...
  async onUserLogout(): Promise<void> {
    try {
      ...
      try {
        (settingsService as any).isInitialized = false;
      } catch (_) {}
   ...
   */
  async notifyStorageModeChange(mode: 'server' | 'local'): Promise<void> {
    try {
      // 这里可以添加存储模式切换的额外逻辑
      // 比如清理缓存、预加载数据等
   ...
   */
  addListener(listener: (event: 'login' | 'logout', data?: any) => void): void {
   ...
   * @param {Function} listener - 监听器函数
   */
  removeListener(listener: (event: 'login' | 'logout', data?: any) => void): void {
   ...
   */
  notifyListeners(event: 'login' | 'logout', data?: any): void {
  ...
  async refreshLoginStatus(): Promise<void> {
    try {
      ...
      this.isLoggedIn = (userStore as any).isLoggedIn as boolean;
      this.currentUser = (userStore as any).user;
