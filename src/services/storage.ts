...
class StorageService {
  ...
  private prefixedStorage: ReturnType<typeof storageUtils.createPrefixedStorage>;
   ...
   */
  setItem(key: string, value: any, options: { expiry?: number } = {}) {
    try {
      const storageOptions: { expiration?: number } = {};
      if (options.expiry) {
  ...
  getItem<T = any>(key: string, defaultValue: T | null = null): T | null {
    try {
      return this.prefixedStorage.get(key, defaultValue) as T | null;
    } catch (error) {
   ...
   */
  clear(clearAll: boolean = false): boolean {
    try {
      if (clearAll) {
