...
export function formatDate(date: Date | string | number, format: string = 'YYYY-MM-DD HH:mm:ss'): string {
  ...
  for (const key in options) {
    ...
    if (regex.test(result)) {
      const execRes = regex.exec(result);
      if (!execRes) continue;
      const match = execRes[1];
 ...
 */
export function debounce<T extends any[]>(fn: (...args: T) => any, wait: number = 300) {
 ...
 */
export function throttle<T extends any[]>(fn: (...args: T) => any, wait: number = 300) {
  let last = 0;
  let timeout: number | null = null;
 ...
 */
export async function copyToClipboard(text: string, showMessage: boolean = true): Promise<boolean> {
  try {
