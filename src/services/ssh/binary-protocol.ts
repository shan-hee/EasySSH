...
export class BinaryMessageEncoder {
  ...
  static encode(
    ...
    try {
      ...
      // 写入Payload Data (如果存在)
      if (payloadData) {
        const payloadArray = new Uint8Array(payloadData as ArrayBuffer | Uint8Array);
...
export class BinaryMessageDecoder {
  ...
  static decode(messageBuffer: ArrayBuffer): {
    ...
    try {
      ...
      // 读取Payload Data (如果存在)
      let payloadData: ArrayBuffer | null = null;
...
export class BinaryMessageSender {
  ...
  static send(
    ...
    try {
      ...
      log.error('发送二进制消息失败:', error as any);
      throw error;
    }
  }
   ...
   */
  static sendSSHData(ws: WebSocket, sessionId: string, data: string | Uint8Array | ArrayBuffer): void {
    const headerData = {
      sessionId,
      timestamp: Date.now(),
      dataType: 'terminal'
    ...
    if (typeof data === 'string') {
      ...
      payloadData = new Uint8Array(data as ArrayBuffer);
    }
   ...
   */
  static sendSSHResize(ws: WebSocket, sessionId: string, cols: number, rows: number): void {
    const headerData = {
      sessionId,
      cols,
      rows,
   ...
   */
  static sendSSHCommand(ws: WebSocket, sessionId: string, command: string): void {
    const headerData = {
      sessionId,
      timestamp: Date.now()
    };
...
export class LegacyDataHandler {
   ...
   */
  static decodeTerminalData(base64Data: string): string {
    try {
      ...
      log.error('Base64解码失败:', error as any);
...
export class UnifiedBinaryHandler {
  private messageHandlers: Map<number, (headerData: any, payloadData: ArrayBuffer | null) => unknown | Promise<unknown>>;
  ...
  registerHandler(
    messageType: number,
    handler: (headerData: any, payloadData: ArrayBuffer | null) => unknown | Promise<unknown>
  ): void {
   ...
   * @param {ArrayBuffer} messageBuffer - 消息缓冲区
   */
  async handleMessage(messageBuffer: ArrayBuffer): Promise<void> {
    try {
      ...
      log.debug('收到统一二进制消息', {
        ...
        sessionId: (headerData as any).sessionId,
        operationId: (headerData as any).operationId,
        payloadSize: payloadData ? payloadData.byteLength : 0
      });
      ...
      log.error('处理二进制消息失败:', error as any);
    }
  }
  ...
  async handleDecodedMessage(decodedMessage: {
    ...
    try {
      ...
      log.error('处理已解码消息失败:', error as any);
    }
  }
}
