// Types enabled: this file was previously skipped by the compiler. Add explicit types for clarity.
import log from '../log';
...
import type {
  SftpBinaryEventDetail,
  SftpSuccessHeader,
  SftpErrorHeader,
  SftpProgressHeader,
  SftpFileDataHeader,
  SftpFolderDataHeader
} from '@/types/sftp';
...
type SftpOperation = {
  ...
  progress?: (progress: number, meta?: any) => void;
  type: string;
  meta?: any;
  sshSessionId?: string;
};

type SftpSessionInfo = {
  id: string;
  sshSessionId: string;
  currentPath: string;
  isActive: boolean;
...
class SFTPService {
  sshService: any;
  activeSftpSessions: Map<string, SftpSessionInfo>;
  fileOperations: Map<string, SftpOperation>;
  operationId: number;
  _canceledOps: Set<string>;
  chunkSize: number;
  transferTimeout: number;
  chunkReassembler: Map<string, any>;
  _folderDownloads: Map<string, any>;
  _progressLogState: Map<string, any>;
  _progressState: Map<string, any>;
  cancelledOperationCounts?: Map<string, number>;
  constructor(sshService: any) {
    ...
    // Optional local counters for cancelled operations (used by some cleanup paths)
    this.cancelledOperationCounts = new Map();

    // 监听SFTP二进制消息事件
    window.addEventListener('sftp-binary-message', (event: Event) => {
      const detail = (event as CustomEvent<SftpBinaryEventDetail>).detail;
      this.handleBinaryMessage(detail);
    });
   ...
   */
  _createSftpMessage(type: string, data: any): any {
    return {
      type,
      data,
      timestamp: Date.now(),
   ...
   * 二进制消息编码器
   */
  static _encodeBinaryMessage(messageType: number, headerData: any, payloadData: ArrayBuffer | Uint8Array | null = null): ArrayBuffer {
   ...
   * 二进制消息解码器
   */
  static _decodeBinaryMessage(messageBuffer: ArrayBuffer): any {
    const MAGIC_NUMBER = 0x45535348; // "ESSH"
    const VERSION = 0x02;
   ...
   * 计算SHA256校验和
   */
  static async _calculateSHA256(data: ArrayBuffer): Promise<string> {
   ...
   */
  async createSftpSession(sessionId: string): Promise<string> {
    if (!sessionId) {
      throw new Error('创建SFTP会话失败: 缺少会话ID');
    }

    // 通过终端ID获取实际的SSH会话ID
    let sshSessionId: string = sessionId;
    ...
    if (!sessionId.startsWith('ssh_') && this.sshService.sessions.size > 0) {
      ...
      if (sshSessionId === sessionId) {
        if (this.sshService.sessions.size === 1) {
          sshSessionId = String(Array.from(this.sshService.sessions.keys())[0] ?? sessionId);
    ...
    try {
      ...
      if (session.socket && session.socket.readyState === WS_CONSTANTS.OPEN) {
        return new Promise((resolve, reject) => {
          ...
          // 创建二进制SFTP初始化消息处理器
          const handleSftpInitResponse = (event: Event) => {
            // 检查是否是SFTP初始化成功的二进制消息事件
            if (event.type === 'sftp-binary-message') {
              const { messageType, headerData } = (event as CustomEvent).detail || {} as any;
          ...
          // 添加二进制消息监听器
          window.addEventListener('sftp-binary-message', handleSftpInitResponse as EventListener);
   ...
   */
  async closeSftpSession(sessionId: string): Promise<boolean> {
    if (!sessionId) {
   ...
   */
  async listDirectory(sessionId: string, path: string = '.'): Promise<any[]> {
  ...
  async uploadFileBinary(
    ...
    file: File,
    remotePath: string,
    progressCallback: (progress: number, meta?: any) => void,
    options: any = {}
  ): Promise<any> {
    await this._ensureSftpSession(sessionId);
    ...
    return new Promise((resolve, reject) => {
      ...
      try {
        ...
        reader.onload = async (event: any) => {
          try {
            let fileBuffer: ArrayBuffer = (event.target as any)?.result as ArrayBuffer;
            ...
            if (timeout) clearTimeout(timeout);
            this.fileOperations.delete(operationId);
            reject(new Error(`处理文件失败: ${(error as any)?.message}`));
          }
        };
   ...
   * @private
   */
  async _uploadFileInChunks(session: any, baseMetadata: any, fileBuffer: ArrayBuffer, _progressCallback: any, signal?: AbortSignal): Promise<void> {
   ...
   * @returns {Promise<Object>} - 上传结果
   */
  async uploadFile(sessionId: string, file: File, remotePath: string, progressCallback: (p: any) => void, options: any = {}): Promise<any> {
   ...
   */
  async getFileContent(sessionId: string, remotePath: string): Promise<string> {
    await this._ensureSftpSession(sessionId);
    ...
    try {
      ...
      return new Promise<string>((resolve, reject) => {
        ...
        reader.onload = (e: ProgressEvent<FileReader>) => {
          try {
            ...
            reject(new Error(`处理文件内容失败: ${(error as any)?.message}`));
          }
        };
      ...
      throw new Error(`获取文件内容失败: ${(error as any)?.message || '未知错误'}`);
    }
  }
   ...
   */
  async downloadFolderBinary(
    sessionId: string,
    remotePath: string,
    progressCallback: (p: any, meta?: any) => void,
    options: any = {}
  ): Promise<any> {
    await this._ensureSftpSession(sessionId);
    ...
    // 暴露operationId以便外部取消
    (promise as any).operationId = operationId;
   ...
   */
  async saveFileContent(sessionId: string, remotePath: string, content: string): Promise<boolean> {
    await this._ensureSftpSession(sessionId);
    ...
    try {
      ...
      throw new Error(`保存文件内容失败: ${(error as any)?.message || '未知错误'}`);
    }
  }
   ...
   */
  async createFile(sessionId: string, remotePath: string): Promise<any> {
    await this._ensureSftpSession(sessionId);
   ...
   * @returns {Promise<Object>} - 创建结果
   */
  async createDirectory(sessionId: string, remotePath: string): Promise<any> {
    await this._ensureSftpSession(sessionId);
   ...
   */
  async deleteFile(sessionId: string, remotePath: string): Promise<any> {
    await this._ensureSftpSession(sessionId);
   ...
   * @returns {Promise<Object>} - 删除结果
   */
  async delete(sessionId: string, remotePath: string, isDirectory: boolean = false): Promise<any> {
    await this._ensureSftpSession(sessionId);
   ...
   * @returns {Promise<Object>} - 删除结果
   */
  async fastDeleteDirectory(sessionId: string, remotePath: string): Promise<any> {
    await this._ensureSftpSession(sessionId);
   ...
   */
  async changePermissions(sessionId: string, remotePath: string, permissions: number): Promise<any> {
    await this._ensureSftpSession(sessionId);
   ...
   */
  async rename(sessionId: string, oldPath: string, newPath: string): Promise<any> {
    await this._ensureSftpSession(sessionId);
   ...
   */
  async cancelUpload(sessionId: string, operationId: string): Promise<any> {
    await this._ensureSftpSession(sessionId);
   ...
   * @private
   */
  async _ensureSftpSession(sessionId: string): Promise<void> {
    if (!sessionId) {
   ...
   * @private
   */
  _getSSHSession(sessionId: string): { sshSessionId: string; session: any } {
    // 获取SFTP会话
    const sftpSession = this.activeSftpSessions.get(sessionId);
    if (!sftpSession) {
   ...
   */
  setupBinaryMessageHandler(socket: WebSocket): void {
    // 添加二进制消息处理器
    socket.addEventListener('message', event => {
      if (event.data instanceof ArrayBuffer) {
        try {
          const decoded = SFTPService._decodeBinaryMessage(event.data);
          const detail = {
            messageType: decoded.messageType,
            headerData: decoded.headerData,
            payloadData: decoded.payloadData
          } as SftpBinaryEventDetail;
          this.handleBinaryMessage(detail);
        } catch (_) {
   ...
   */
  handleBinaryMessage(messageDetail: SftpBinaryEventDetail): void {
    try {
   ...
   * @private
   */
  _handleBinarySuccess(headerData: SftpSuccessHeader, payloadData: ArrayBuffer | Uint8Array | null): void {
   ...
   * @private
   */
  _handleBinaryError(headerData: SftpErrorHeader): void {
   ...
   * @private
   */
  _handleBinaryProgress(headerData: SftpProgressHeader): void {
    const { operationId } = headerData;
    ...
    try {
      ...
      const clampPct = (v: number, completed: boolean): number => {
      ...
      let safeProgress =
        derived !== undefined ? clampPct(derived, completed) : clampPct(progress ?? 0, completed);
    ...
    if (operation && operation.progress && typeof operation.progress === 'function') {
      ...
      operation.progress(headerData.progress ?? 0, headerData);
    }
  }
   ...
   * @private
   */
  _handleBinaryFileData(headerData: SftpFileDataHeader, payloadData: ArrayBuffer | Uint8Array | null): void {
    const { operationId } = headerData;
    const operation = this.fileOperations.get(operationId);
    ...
    if (operation && operation.resolve) {
      try {
        ...
        if (!payloadData) {
          ...
          blobData = new Uint8Array(payloadData as ArrayBuffer);
        }

        // 创建文件Blob
        // Use only the view's byte range to avoid packing unused tail from the backing buffer
        const out = new Uint8Array(blobData.byteLength);
        out.set(blobData);
        ...
        if (operation.reject) {
          operation.reject(new Error(`处理文件数据失败: ${(error as any)?.message}`));
          this.fileOperations.delete(operationId);
        }
      }
    }
   ...
   * @private
   */
  _handleBinaryFolderData(headerData: SftpFolderDataHeader, payloadData: ArrayBuffer | Uint8Array | null): void {
    const { operationId } = headerData;
    const operation = this.fileOperations.get(operationId);
    ...
    if (operation && operation.resolve) {
      try {
        ...
        if (operation.reject) {
          operation.reject(new Error(`处理文件夹数据失败: ${(error as any)?.message}`));
          this.fileOperations.delete(operationId);
        }
      }
    }
