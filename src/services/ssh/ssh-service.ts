...
import type {
  PingHeader,
  PongHeader,
  ConnectionRegisteredHeader,
  ConnectedHeader,
  NetworkLatencyHeader,
  SSHDataHeader,
  SSHDataAckHeader
} from '@/types/binary';
import type { SshConnectedDetail, SshSessionCreationFailedDetail, NetworkLatencyDetailEvent } from '@/types/events';

/**
 * SSH服务模块，负责管理SSH连接和终端会话
 */
class SSHService {
  // Keep broad "any" for session payloads to avoid over-constraining runtime data
  sessions: Map<string, SSHSession>;
  terminals: Map<string, TerminalLike>;
  isReady: boolean;
  isInitializing: boolean;
  keepAliveIntervals: Map<string, KeepAliveData>;
  latencyTimers: Map<string, ReturnType<typeof setInterval>>;
  terminalSessionMap: Map<string, string>;
  sessionTerminalMap: Map<string, string>;
  pendingTerminalCancels: Map<string, PendingTerminalCancel>;
  _pendingQuietCloses: WeakMap<WebSocket, true>;
  connectionTimeoutHandles: Map<string, number>;
  verboseLogging: { buffer: boolean; abort: boolean; wsData: boolean };
  binaryHandler: UnifiedBinaryHandler;
  ipv4Url: string;
  ipv6Url: string;
  baseUrl: string;
  ...
  latencyData: Map<string, LatencyDetail>;
  dynamicConfig: ReturnType<typeof getDynamicConstants> | null;
  cleanupInterval: number;
  connectionIdExpiration: number;
  instance: this;
  pendingConnections?: Map<string, PendingConnectionRecord>;
  constructor() {
   ...
   * 设置统一二进制处理器
   */
  _setupBinaryHandlers(): void {
    try {
      // 注册新的控制消息处理器
      this.binaryHandler.registerHandler(BINARY_MESSAGE_TYPES.PING, (headerData: PingHeader, _payloadData: any) => {
        this._handleBinaryPing(headerData);
      });

      this.binaryHandler.registerHandler(BINARY_MESSAGE_TYPES.PONG, (headerData: PongHeader, _payloadData: any) => {
        this._handleBinaryPong(headerData);
      });

      this.binaryHandler.registerHandler(
        BINARY_MESSAGE_TYPES.CONNECTION_REGISTERED,
        (headerData: ConnectionRegisteredHeader, _payloadData: any) => {
      ...
      this.binaryHandler.registerHandler(
        ...
        (headerData: ConnectedHeader, _payloadData: any) => {
      ...
      this.binaryHandler.registerHandler(
        ...
        (headerData: NetworkLatencyHeader, _payloadData: any) => {
      ...
      this.binaryHandler.registerHandler(
        ...
        (headerData: SSHDataHeader, payloadData: any) => {
      ...
      this.binaryHandler.registerHandler(
        ...
        (headerData: SSHDataAckHeader, _payloadData: any) => {
      ...
      this.binaryHandler.registerHandler(
        ...
        (headerData, payloadData) => {
          // 通过事件系统转发给SFTP服务
          window.dispatchEvent(new CustomEvent('sftp-binary-message', { detail: { headerData, payloadData, messageType: BINARY_MESSAGE_TYPES.SFTP_SUCCESS } }));
        }
      );
      ...
      this.binaryHandler.registerHandler(
        ...
        (headerData, payloadData) => {
          window.dispatchEvent(new CustomEvent('sftp-binary-message', { detail: { headerData, payloadData, messageType: BINARY_MESSAGE_TYPES.SFTP_ERROR } }));
        }
      );
      ...
      this.binaryHandler.registerHandler(
        ...
        (headerData, payloadData) => {
          window.dispatchEvent(new CustomEvent('sftp-binary-message', { detail: { headerData, payloadData, messageType: BINARY_MESSAGE_TYPES.SFTP_PROGRESS } }));
        }
      );
      ...
      this.binaryHandler.registerHandler(
        ...
        (headerData, payloadData) => {
          window.dispatchEvent(new CustomEvent('sftp-binary-message', { detail: { headerData, payloadData, messageType: BINARY_MESSAGE_TYPES.SFTP_FILE_DATA } }));
        }
      );
      ...
      this.binaryHandler.registerHandler(
        ...
        (headerData, payloadData) => {
          window.dispatchEvent(new CustomEvent('sftp-binary-message', { detail: { headerData, payloadData, messageType: BINARY_MESSAGE_TYPES.SFTP_FOLDER_DATA } }));
        }
      );
   ...
   * 处理SSH二进制数据
   */
  _handleBinarySSHData(headerData: SSHDataHeader, payloadData: ArrayBuffer | Uint8Array): void {
    ...
    if (!session.terminal) {
      // 终端未就绪：缓存数据并仅在首次缓存时提示一次，设置上限以防内存增长
      if (!session.buffer) {
        session.buffer = '';
      ...
      session.bufferSize = (session.bufferSize ?? 0) + dataStr.length;

      const MAX_BUFFER_SIZE = 512 * 1024; // 512KB，与JSON路径保持一致
      if ((session.bufferSize ?? 0) > MAX_BUFFER_SIZE) {
   ...
   * 处理SSH数据确认
   */
  _handleSSHDataAck(headerData: SSHDataAckHeader): void {
   ...
   * 记录会话活动时间
   */
  _recordSessionActivity(session: SSHSession): void {
    if (session) {
   ...
   */
  async createSession(connection: ConnectionConfig): Promise<string> {
   ...
   * @returns {Promise<string>} - 会话ID
   */
  async _createNewSession(connection: ConnectionConfig, providedSessionId?: string): Promise<string> {
    try {
      ...
      if (pendingCancel) {
        if (terminalId != null) this.pendingTerminalCancels.delete(terminalId as string);
      }
      ...
      if (terminalId && !isExpectedClosure) {
        window.dispatchEvent(
          new CustomEvent<SshSessionCreationFailedDetail>('ssh-connection-failed', {
            detail: {
        ...
        window.dispatchEvent(
          new CustomEvent<SshSessionCreationFailedDetail>(EVENTS.SSH_SESSION_CREATION_FAILED, {
            detail: {
              sessionId,
              terminalId,
      ...
      const terminalId = connection?.terminalId
        ...
        : (providedSessionId ? this.sessionTerminalMap.get(providedSessionId) : undefined);
      const pendingCancel = terminalId && this.pendingTerminalCancels.has(terminalId);
      const expectedCancellationFlag = !!(
        error &&
        typeof error === 'object' &&
        (error as any).expectedCancellation
      );
      const expected = pendingCancel || expectedCancellationFlag || this._isExpectedClosure(error);

      if (pendingCancel) {
        if (terminalId) if (terminalId != null) this.pendingTerminalCancels.delete(terminalId as string);
      ...
      if (expected) {
        const originalReason =
          typeof error === 'object' && error !== null && (error as any).originalMessage
            ? `，原始原因: ${(error as any).originalMessage}`
            : '';
  ...
  async _createSessionWithUrl(
    ...
    try {
      ...
      let propagatedError: any;

      if (pendingCancel) {
        propagatedError = new Error('连接已取消');
        (propagatedError as any).expectedCancellation = true;
        (propagatedError as any).originalMessage = originalMessage;
        (propagatedError as any).cancellationReason = pendingCancelInfo.reason || 'frontend_monitor_unsubscribed';
        (propagatedError as any).terminalId = terminalId;
        (propagatedError as any).originalError = error;
        if (terminalId != null) this.pendingTerminalCancels.delete(terminalId as string);
      ...
      const isExpectedClosure =
        pendingCancel ||
        (propagatedError && (propagatedError as any).expectedCancellation) ||
      ...
      socket.onerror = (event: any) => {
        ...
        if (this.sessions.has(sessionId)) {
          ...
          if (session.retryCount < this.reconnectAttempts) {
            ...
            setTimeout(() => {
              this._reconnectSession(sessionId, connection).catch((error: any) => {
        ...
        // 清理任何挂起的 pendingConnections（按 sessionId 匹配）
        try {
          if (this.pendingConnections && this.pendingConnections.size) {
            for (const [connId, data] of this.pendingConnections.entries()) {
              if (data && data.sessionId === sessionId) {
                try { data.reject && data.reject(new Error('连接错误')); } catch (_) {}
      ...
      socket.onclose = (event: CloseEvent) => {
        ...
        try {
          if (this.pendingConnections && this.pendingConnections.size) {
            for (const [connId, data] of this.pendingConnections.entries()) {
              if (data && data.sessionId === sessionId) {
                try { data.reject && data.reject(new Error('连接已关闭')); } catch (_) {}
                this.pendingConnections.delete(connId);
              }
            }
          }
      ...
      socket.onmessage = async (event: MessageEvent) => {
        try {
          ...
          if (event.data instanceof ArrayBuffer) {
            await this._handleBinaryMessage(event.data, sessionId);
            return;
          }
          ...
          switch (message.type) {
            case MESSAGE_TYPES.CONNECTED: {
              ...
              const sshConnectedEvent: CustomEvent<SshConnectedDetail> = new CustomEvent(EVENTS.SSH_CONNECTED, {
                detail: {
                  sessionId,
                  host: connHost,
          ...
          break;
            ...
            case MESSAGE_TYPES.NETWORK_LATENCY:
              try {
                ...
                let terminalId: string | null = this.sessionTerminalMap.get(sessionId) ?? null;
                ...
                // 触发全局网络延迟事件
                window.dispatchEvent(
                  new CustomEvent<NetworkLatencyDetailEvent>(LATENCY_EVENTS.GLOBAL, {
                ...
                window.dispatchEvent(
                  new CustomEvent<NetworkLatencyDetailEvent>(LATENCY_EVENTS.TOOLBAR, {
                    detail: enrichedLatencyDetail
                  })
                );
                ...
                if (terminalId) {
                  window.dispatchEvent(
                    new CustomEvent<NetworkLatencyDetailEvent>(LATENCY_EVENTS.TERMINAL, {
                      detail: enrichedLatencyDetail
                    })
                  );
                }
   ...
   * 设置会话保活机制
   */
  _setupKeepAlive(sessionId: string): void {
    this._clearKeepAlive(sessionId);

    const pingRequests = new Map<string, { timestamp: Date; sessionId: string; clientSendTime?: number; isParallelPingMeasurement?: boolean }>();
    ...
    const keepAlive: KeepAliveData = { interval, pingRequests };
    this.keepAliveIntervals.set(sessionId, keepAlive);
  }

  /**
   * 清除会话保活机制
   */
  _clearKeepAlive(sessionId: string): void {
    if (this.keepAliveIntervals.has(sessionId)) {
      ...
      if (keepAliveData.interval) {
        clearInterval(keepAliveData.interval as number);
      }
   ...
   */
  _triggerImmediatePing(sessionId: string): void {
    if (!this.sessions.has(sessionId)) {
      return;
    }
   ...
   * 处理WebSocket关闭码和原因
   */
  _getCloseReasonText(code: number, reason?: string): string {
    const closeCodeMap: { [key: number]: string } = {
      1000: '正常关闭',
      1001: '终端关闭',
      1002: '协议错误',
      1003: '数据类型错误',
   ...
   * 尝试重新连接会话
   */
  async _reconnectSession(sessionId: string, connection: any): Promise<void> {
    if (!this.sessions.has(sessionId)) {
    ...
    try {
      ...
      log.warn(`IPv4重连失败: ${(ipv4Error as any)?.message}，尝试IPv6连接`);
      ...
      try {
        ...
        session.isReconnecting = false;
        log.error(`IPv6重连也失败: ${(ipv6Error as any)?.message}`);
        throw new Error(
          `重连失败: IPv4(${(ipv4Error as any)?.message}) 和 IPv6(${(ipv6Error as any)?.message})`
        );
      }
    }
  }
   ...
   */
  _generateSecureConnectionId(_connection: ConnectionConfig): string {
    // 生成随机ID
   ...
   * 清理过期的待处理连接
   */
  _cleanupPendingConnections(): void {
   ...
   * 处理终端输入
   */
  _processTerminalInput(session: SSHSession, data: string | Uint8Array | ArrayBuffer): void {
    if (!session) {
   ...
   */
  setVerboseLogging(options: Partial<{ buffer: boolean; abort: boolean }> = {}): void {
   ...
   * 关闭会话
   */
  async cancelConnectionByTerminal(
    terminalId: string,
    reason: string = 'frontend_monitor_unsubscribed'
  ): Promise<boolean> {
    if (!terminalId) {
  ...
  async closeSession(
    sessionId: string,
    options: { reason?: string; detail?: Record<string, unknown> } = {}
  ): Promise<boolean> {
    if (!sessionId) {
   ...
   * 发送自定义错误事件
   */
  _dispatchErrorEvent(sessionId: string, message: string): void {
    try {
      window.dispatchEvent(
   ...
   * 彻底释放会话相关的所有资源
   */
  releaseResources(sessionId: string): boolean {
    try {
   ...
   */
  getNetworkLatency(sessionId: string): LatencyDetail | null {
   ...
   * @returns {Object|null} - 延迟数据或null
   */
  getTerminalNetworkLatency(terminalId: string): LatencyDetail | null {
    const sessionId = this.terminalSessionMap.get(terminalId);
    if (sessionId) {
   ...
   * @param {string} sessionId - 会话ID
   */
  setTerminalSessionMapping(terminalId: string, sessionId: string): void {
   ...
   */
  _translateErrorMessage(errorMessage: string): string {
    ...
    // 寻找完全匹配的错误消息
    if (Object.prototype.hasOwnProperty.call(errorTranslations, translatedMessage)) {
      return (errorTranslations as Record<string, string>)[translatedMessage];
    }
   ...
   */
  _getErrorMessage(error: any): string {
   ...
   * @returns {boolean}
   */
  _isExpectedClosure(error: unknown): boolean {
   ...
   */
  _closeSocketQuietly(socket: WebSocket, reason: string = 'client_close'): void {
    if (!socket) return;
  ...
  _clearConnectionTimeout(sessionId: string): void {
    if (!sessionId) return;
   ...
   */
  _encryptSensitiveData(data: string, key: string): string {
    try {
      if (!data) return '';
   ...
   */
  getTerminalIdBySession(sessionId: string): string | null {
    if (!sessionId) {
      return null;
    }
   ...
   */
  _createStandardMessage(type: string, data: any, requestId: string | null = null): any {
    const message: any = {
      type,
      data,
      timestamp: Date.now(),
   ...
   * @param {string} sessionId 会话ID
   */
  async _handleBinaryMessage(buffer: ArrayBuffer, sessionId: string): Promise<void> {
    try {
      ...
      if (this.binaryHandler) {
        try {
          await this.binaryHandler.handleDecodedMessage(decoded);
        } catch (handlerErr) {
          log.error('二进制消息处理器执行失败', handlerErr);
        }
      }
    } catch (error) {
      log.error('处理二进制消息失败', { error: (error as any)?.message, sessionId });
    }
  }
   ...
   */
  _handleBinaryPing(headerData: any): void {
   ...
   * @param {Object} headerData 头部数据
   */
  _handleBinaryPong(headerData: any): void {
    const { sessionId } = headerData;
   ...
   * @param {Object} headerData 头部数据
   */
  _handleBinaryConnectionRegistered(headerData: any): void {
   ...
   * @param {Object} headerData 头部数据
   */
  _handleBinaryConnected(headerData: any): void {
    ...
    if (connectionId && this.pendingConnections && this.pendingConnections.has(connectionId)) {
      const pendingData = this.pendingConnections.get(connectionId)!;
      if (pendingData && pendingData.resolve) {
   ...
   * @param {Object} headerData 头部数据
   */
  _handleBinaryNetworkLatency(headerData: any): void {
   ...
   */
  _triggerConnectionSuccess(sessionId: string, serverInfo: any = {}): void {
    try {
      const session = this.sessions.get(sessionId)!;
      if (!session) return;

      const connection: any = session.connection || {};
      ...
      if (connHost) {
        ...
        if (!terminalId && connection.terminalId) {
          const tId = String(connection.terminalId);
          this.sessionTerminalMap.set(sessionId, tId);
          this.terminalSessionMap.set(tId, sessionId);
          terminalId = tId;
        }
   ...
   * @param {string} sessionId 会话ID
   */
  _setupLatencyTimer(sessionId: string): void {
    if (!sessionId || !this.sessions.has(sessionId)) {
      return;
    }
   ...
   * @param {string} sessionId 会话ID
   */
  _clearLatencyTimer(sessionId: string): void {
   ...
   * @param {string} sessionId 会话ID
   */
  _triggerLatencyMeasurement(sessionId: string): void {
    if (!this.sessions.has(sessionId)) {
      return;
    }
