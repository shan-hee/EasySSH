...
class PerformanceService {
  ...
  private samples: { eventLoopLag: number[]; fps: number[]; memory: Array<{ used: number; total: number; limit: number; timestamp: number }> };
  private maxSamples: number;
  private _lagTimer: number | null;
  private _memTimer: number | null;
  private _rafId: number | null;
  private _fpsLastTime: number;
  private _fpsFrames: number;
  private _fpsWindowMs: number;
  ...
  init(): boolean {
    ...
    this._lagTimer = window.setInterval(() => {
    ...
    // 内存使用（如浏览器支持）
    this._memTimer = window.setInterval(() => {
      try {
        const m = (performance as any) && (performance as any).memory;
  ...
  stop(): void {
    ...
    this.started = false;
    if (this._lagTimer) window.clearInterval(this._lagTimer);
    if (this._memTimer) window.clearInterval(this._memTimer);
  ...
  /** 推入样本并裁剪长度 */
  _pushSample(arr: any[], v: any): void {
    arr.push(v);
  ...
  getReport() {
    ...
    const report: {
      ...
      fps: any;
      memory: any;
      recommendations: Array<{ type: 'performance' | 'memory' | 'load'; priority: 'high' | 'medium' | 'low'; message: string }>;
    } = {
