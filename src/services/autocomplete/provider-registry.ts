...
export type ProviderOptions = {
  ...
  enableScriptCompletion?: boolean;
  [key: string]: unknown;
};
...
export interface AutocompleteProvider {
  id?: string;
  priority?: number;
  supports?: (context: AutocompleteContext, options: ProviderOptions) => boolean;
  suggest: (
...
const _providers: AutocompleteProvider[] = [];

function register(provider: AutocompleteProvider) {
...
/**
 * 聚合收集建议
 * @param context 统一上下文（来自 terminal-autocomplete）
 * @param options 可选项（如最大数量/开关）
 * @returns 建议集合
 */
...
function collect(
  ...
  for (const p of _providers) {
    try {
      if (typeof p.supports === 'function' && !p.supports(context, opts)) continue;
      const result = p.suggest(context, opts) as SuggestionItem[];
      if (Array.isArray(result) && result.length) all.push(...result);
    } catch (e) {
      log.debug(`Provider 失败: ${p?.id || 'unknown'}`, e);
    }
