...
import type { AutocompleteContext, ProviderOptions, SuggestionItem } from '../provider-registry';
...
export default class HistoryProvider {
  ...
  suggest(context: AutocompleteContext, options: ProviderOptions): SuggestionItem[] {
    ...
    // 读取个性化仓库（只读）
    const cache = (storageService.getItem(STORE_KEY, null) as any) || { items: {} };
    const items = cache.items || {};

    // 评分：优先 selects，其次 exposures，最近选择时间加权
    const scored: Array<SuggestionItem & { key: string; score: number }> = [];
    for (const key of Object.keys(items)) {
      const it = items[key] || {};
      let text = '';
      let type: string = 'commands';
      let category: string = 'commands';
      let desc = '';
      ...
      if (key.startsWith('phrase:')) {
        ...
        const script = (scriptLibraryService as any).getScriptById(id, 'public') ||
          (scriptLibraryService as any).getScriptById(id, 'user');
