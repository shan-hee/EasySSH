...
export default class FigProvider {
  ...
  _findCmdDef(name) {
    try {
      ...
      return linuxCommands.find(c => c.name && c.name.toLowerCase() === low) || null;
    } catch (_) {
      return null;
    }
  }
  ...
  suggest(context, options) {
    ...
    const idx = typeof context?.wordPosition === 'number' ? context.wordPosition : (tokens.length ? tokens.length - 1 : 0);
    const atStart = idx === 0;
    ...
    try {
      ...
      if (atStart && !input.startsWith('-')) {
        const source = Array.from(new Set([...(figIndex || []), ...linuxCommands.map(c => c.name)]));
        for (const nameRaw of source) {
          ...
          if (nameLow.startsWith(key) || nameLow.includes(key)) {
            ...
            const lc = linuxCommands.find(c => c.name.toLowerCase() === nameLow);
      ...
      if (head) {
        ...
        if (spec && !input.startsWith('-')) {
          if (idx === 1) {
            const subs = Array.isArray(spec.subcommands) ? spec.subcommands : [];
            for (const sc of subs) {
              ...
              if (nameLow.startsWith(key) || nameLow.includes(key)) {
                out.push({
                  ...
                  text: name,
                  description: (typeof sc === 'object' ? sc?.description : '') || '子命令',
                  type: 'commands',
                  category: 'commands',
            ...
            const sub1 = (Array.isArray(spec.subcommands) ? spec.subcommands : []).find(sc => (typeof sc === 'object' ? (sc.name || '') : String(sc)).toLowerCase() === (tokens[1] || '').toLowerCase());
            const subs2 = (sub1 && Array.isArray(sub1.subcommands)) ? sub1.subcommands : [];
            for (const sc of subs2) {
              const name = typeof sc === 'string' ? sc : sc?.name;
              if (!name) continue;
              const nameLow = name.toLowerCase();
              if (nameLow.startsWith(key) || nameLow.includes(key)) {
        ...
        // 选项（当前输入以 - 开头）
        if (input.startsWith('-')) {
          const sub1 = (spec && Array.isArray(spec.subcommands))
            ? spec.subcommands.find(sc => (typeof sc === 'object' ? (sc.name || '') : String(sc)).toLowerCase() === (tokens[1] || '').toLowerCase())
            : null;
          const sub2 = (sub1 && Array.isArray(sub1.subcommands))
            ? sub1.subcommands.find(sc => (typeof sc === 'object' ? (sc.name || '') : String(sc)).toLowerCase() === (tokens[2] || '').toLowerCase())
            : null;
          const opts = (sub2 && Array.isArray(sub2.options))
            ? sub2.options
            : (sub1 && Array.isArray(sub1.options))
              ? sub1.options
              : (spec && Array.isArray(spec.options))
                ? spec.options
                : (this._findCmdDef(head)?.options || []);
          for (const opt of opts) {
            const optName = Array.isArray(opt?.names) ? opt.names[0] : (opt?.name || String(opt));
            const optStr = String(optName);
            const matches = optStr.startsWith(input) || optStr.includes(input);
            if (!matches) continue;
            const desc = (typeof opt === 'object' ? opt.description : '') || '选项';
            out.push({
