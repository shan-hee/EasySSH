...
const modules: Record<string, any> = import.meta.glob('../../assets/fig-specs/*.json', {
  eager: true
}) as any;

const _specs: Map<string, any> = new Map();
...
export function listCommands(): string[] {
  return Array.from(_names.values()) as string[];
}

export function getSpec(name?: string | null): any | null {
  if (!name) return null;
