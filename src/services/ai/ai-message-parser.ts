...
type ParsedPart =
  ...
  | { type: 'code'; language: string; content: string; isExecutable: boolean };

interface ParseAIMessageResult {
 ...
 * @returns {Object} 解析结果
 */
export function parseAIMessage(content: string): ParseAIMessageResult {
 ...
 */
function parseContentWithCodeBlocks(content: string): ParsedPart[] {
 ...
 * @returns {Object} 验证结果
 */
export function validateCommand(command: string): { isValid: boolean; reason?: string; severity?: 'high' | 'medium' } {
  if (!command) {
 ...
 */
export function formatCommandForDisplay(command: string, options: FormatOptions = {}): string {
  const {
