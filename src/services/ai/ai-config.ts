...
type AIConfigData = {
  enabled: boolean;
  provider: 'openai' | 'azure' | 'custom' | string;
  baseUrl: string;
  apiKey: string;
  model: string;
  ...
  proxyUrl?: string;
  ui?: { idleDelay?: number };
  security?: { maxContextLines?: number };
};

class AIConfig {
  private storageKey: string;
  private initialized: boolean;
  public config: AIConfigData;
   ...
   */
  async load(options: { forceServer?: boolean } = {}): Promise<AIConfigData> {
    try {
      // 确保存储服务已初始化
   ...
   */
  async loadSoft(): Promise<AIConfigData> {
    try {
   ...
   */
  async save(newConfig: Partial<AIConfigData>): Promise<boolean> {
    try {
      // 验证配置
      ...
      // 保存到统一存储服务
      await (storageAdapter as any).set(this.storageKey, this.config);
  ...
  set(key: string, value: any): void {
    try {
      const keys = key.split('.');
      let target: any = this.config as any;
   ...
   * @returns {Object} 验证结果
   */
  validate(config: Partial<AIConfigData>): { valid: boolean; error?: string } {
    try {
      ...
      if (config.ui) {
        if (config.ui.idleDelay !== undefined) {
          if (
            typeof config.ui.idleDelay !== 'number' ||
            config.ui.idleDelay < 100 ||
            config.ui.idleDelay > 2000
          ) {
      ...
      if (config.security) {
        if (config.security.maxContextLines !== undefined) {
          if (
            typeof config.security.maxContextLines !== 'number' ||
            config.security.maxContextLines < 50 ||
            config.security.maxContextLines > 500
          ) {
   ...
   * 合并配置对象
   */
  mergeConfig<T extends Record<string, any>>(target: T, source: Partial<T>): T {
    ...
    for (const key in source) {
      if (Object.prototype.hasOwnProperty.call(source, key)) {
        ...
        if (sv && typeof sv === 'object' && !Array.isArray(sv)) {
          result[key] = this.mergeConfig((result as any)[key] || {}, sv);
        } else {
          result[key] = sv;
        }
      }
    ...
    const models = {
      ...
      azure: [
        { value: 'gpt-4', label: 'GPT-4 (Azure)', description: 'Azure OpenAI GPT-4' },
        {
          ...
          description: 'Azure OpenAI GPT-3.5'
        }
      ],
      custom: [{ value: 'custom-model', label: '自定义模型', description: '自定义API兼容模型' }]
    };

    return (models as any)[this.config.provider] || models.openai;
  }
  ...
  isFeatureEnabled(feature: string): boolean {
    return !!(this.get<boolean>(`features.${feature}`, false) && this.get<boolean>('enabled', false));
  }
