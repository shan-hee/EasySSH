...
class AICache {
  ...
  set(key: string, value: any, ttl: number | null = null): void {
    try {
      ...
      const ttlSeconds = typeof ttl === 'number' && ttl > 0 ? ttl : this.defaultTTL;
   ...
   */
  preload(items: Array<{ key: string; value: any; ttl?: number }>): void {
    try {
   ...
   */
  configure(config: { maxSize?: number; defaultTTL?: number }): void {
