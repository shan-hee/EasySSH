...
type LangCode = 'zh-cn' | 'en-us' | 'ja-jp' | 'ko-kr' | 'ascii' | 'unknown';
...
interface LanguageStatusSnapshot {
  serverLanguage: LangCode;
  clientLanguage: string;
  unicodeSupport: boolean;
  shouldUseAsciiMode: boolean;
  recommendedAILanguage: string;
}
...
class LanguageDetector {
   ...
   * @returns {string} 语言代码
   */
  detectClientLanguage(): string {
    try {
      const browserLang = (navigator as any).language || (navigator as any).userLanguage || 'en-US';
   ...
   * @returns {Object} 检测结果
   */
  detectServerLanguage(terminalOutput: string): LanguageDetectResult {
    try {
      ...
      // 检测语言环境变量
      const envPatterns: Record<'zh-cn' | 'en-us' | 'ja-jp' | 'ko-kr', RegExp[]> = {
      ...
      // 检测字符内容
      const charPatterns: Partial<Record<LangCode, RegExp>> = {
        'zh-cn': /[\u4e00-\u9fff]/,
        'ja-jp': /[\u3040-\u309f\u30a0-\u30ff]/,
        'ko-kr': /[\uac00-\ud7af]/
      };
      ...
      // 检测编码
      const encodingPatterns: Record<string, RegExp> = {
        'utf-8': /UTF-?8/i,
        gbk: /GBK/i,
        gb2312: /GB2312/i,
        ascii: /ASCII/i
      ...
      // 环境变量检测
      for (const [lang, patterns] of Object.entries(envPatterns) as [LangCode, RegExp[]][]) {
        for (const pattern of patterns) {
          if (pattern.test(terminalOutput)) {
            result.language = lang as LangCode;
      ...
      // 字符内容检测（优先级高于环境变量）
      for (const [lang, pattern] of Object.entries(charPatterns) as [LangCode, RegExp][]) {
        if (pattern && pattern.test(terminalOutput)) {
          result.language = lang as LangCode;
      ...
      for (const [encoding, pattern] of Object.entries(encodingPatterns)) {
        if (pattern.test(terminalOutput)) {
          result.encoding = encoding as string;
          if (encoding === 'utf-8') {
            result.unicodeSupport = true;
          }
          break;
   ...
   * @returns {string} 推荐语言
   */
  getRecommendedAILanguage(): string {
   ...
   * @returns {Object} 状态信息
   */
  getStatus(): LanguageStatusSnapshot {
    return {
   ...
   */
  toAsciiCompatible(text: string): string {
    if (!this.shouldUseAsciiMode()) {
      return text;
    }
    ...
    try {
      ...
      result = result.replace(/[\s\S]/g, (ch: string) => (ch.charCodeAt(0) <= 0x7f ? ch : '?'));

      return result;
    } catch (error) {
      log.error('ASCII转换失败', error);
      return Array.from(text)
        .map((ch: string) => (ch.charCodeAt(0) <= 0x7f ? ch : '?'))
        .join('');
    }
  }
}
