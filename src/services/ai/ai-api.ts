...
class AIApiService {
  ...
  async getStatus(): Promise<any> {
    try {
      const response = (await apiService.get(`${this.baseUrl}/status`)) as any;
      return response.data;
    } catch (error: unknown) {
      log.error('获取AI服务状态失败', error as any);
      throw error;
    }
  }
   ...
   * @returns {Object} 验证结果
   */
  validateConfig(config: { baseUrl?: string; apiKey?: string; model?: string }): { valid: boolean; errors: string[] } {
    const errors: string[] = [];
