...
class PhraseStoreService {
  ...
  init(): void {
    ...
    try {
      // 首次运行注入默认短句（若未初始化过）
      const inited = !!storageService.getItem(STORAGE_KEYS.INITED, false as any);
      if (!inited) {
        const existingDefaults = storageService.getItem(
          STORAGE_KEYS.DEFAULTS,
          [] as any
        ) as Phrase[];
        const merged = this._mergePhrases(existingDefaults, DEFAULT_PHRASES as Phrase[]);
  ...
  private _mergePhrases(base: Phrase[] = [], extra: Phrase[] = []): Phrase[] {
    const out: Phrase[] = [];
  ...
  getAll(): Phrase[] {
    this.init();
    const defaults = (storageService.getItem(
      STORAGE_KEYS.DEFAULTS,
      [] as any
    ) as Phrase[]) || [];
    const user = (storageService.getItem(STORAGE_KEYS.USER, [] as any) as Phrase[]) || [];
    // 用户短句优先，避免重复
    const seen = new Set<string>();
    const merged: Phrase[] = [];
    for (const p of [...user, ...defaults]) {
      ...
      merged.push({
        text: t,
        description: p?.description || '常用短句',
  ...
  addPhrase(text: string, description: string = '常用短句'): boolean {
    try {
      const t = (text || '').trim();
      if (!t) return false;
  ...
  maybeLearnFromCommand(command: string): boolean {
    try {
