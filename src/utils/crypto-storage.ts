...
export type EncryptedPayload = {
  version: '1.0';
  algorithm: string;
  iv: number[]; // 12-byte IV as array
  salt: number[]; // salt bytes as array
  data: number[]; // ciphertext bytes as array
  timestamp: number;
};
...
class CryptoStorage {
   ...
   */
  async encryptData(password: string, data: any): Promise<EncryptedPayload> {
    if (!this.isSupported()) {
      throw new Error('WebCrypto API不支持');
    }
   ...
   */
  async decryptData(password: string, encryptedData: EncryptedPayload): Promise<any> {
    if (!this.isSupported()) {
      throw new Error('WebCrypto API不支持');
    }
   ...
   */
  async secureStore(key: string, password: string, data: any): Promise<boolean> {
    try {
   ...
   */
  async secureRetrieve(key: string, password: string): Promise<any | null> {
    try {
      const storageKey = this.storagePrefix + key;
   ...
   * @returns {Object} 统计信息
   */
  getStorageStats(): { itemCount: number; totalSize: number; totalSizeKB: number; keys: string[] } {
    const keys = this.listSecureKeys();
    let totalSize = 0;
