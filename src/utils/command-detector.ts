...
export type DetectedCommand = {
  command: string;
  type: 'shell' | 'inline' | string;
  source: 'codeblock' | 'inline' | string;
  language?: string;
  lineNumber?: number;
  startIndex?: number;
  endIndex?: number;
 ...
 */
export function detectCommands(text: string | null | undefined): DetectedCommand[] {
 ...
 * @returns {Array} 命令列表
 */
function detectCodeBlockCommands(text: string): DetectedCommand[] {
  const commands: DetectedCommand[] = [];
  const codeBlockRegex = /```(\w+)?\n?([\s\S]*?)```/g;

  let match: RegExpExecArray | null;
  while ((match = codeBlockRegex.exec(text)) !== null) {
    const language = ((match![1] as string) || 'shell').toLowerCase();
    const code = (match![2] as string).trim();
 ...
 * @returns {Array} 命令列表
 */
function detectInlineCommands(text: string): DetectedCommand[] {
  const commands: DetectedCommand[] = [];
 ...
 * @returns {Array} 命令列表
 */
function parseShellCommands(code: string): DetectedCommand[] {
  const commands: DetectedCommand[] = [];
 ...
 */
function calculateCommandConfidence(command: string): number {
  if (!command) return 0;
 ...
 */
function deduplicateCommands(commands: DetectedCommand[]): DetectedCommand[] {
  const seen = new Set<string>();
  const unique: DetectedCommand[] = [];
 ...
 * @returns {string} 命令类型
 */
export function getCommandType(command: string | null | undefined): string {
