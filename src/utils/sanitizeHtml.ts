...
export function sanitizeHtml(input: string | null | undefined): string {
  try {
    ...
    const allowedTags = new Set<string>(['strong', 'em', 'code', 'a', 'br', 'ul', 'ol', 'li', 'p', 'span']);

    const allowedAttrs: Record<string, Set<string>> = {
      a: new Set<string>(['href', 'target', 'rel'])
    };
    ...
    const sanitizeNode = (node: Node): Node | null => {
      ...
      if (node.nodeType === Node.ELEMENT_NODE) {
        ...
        if (!allowedTags.has(tag)) {
          ...
          (node.childNodes as any as NodeListOf<Node>).forEach((child: Node) => {
    ...
    if (root) {
      (root.childNodes as any as NodeListOf<Node>).forEach((child: Node) => {
        const sanitized = sanitizeNode(child);
