...
type OverrideEntry = { value: any; oldValue: any; timestamp: number };
...
class ConfigManager {
  ...
  constructor() {
    ...
    this.overrides = {} as Record<string, OverrideEntry>;
   ...
   */
  set(path: string, value: any, persist: boolean = false) {
    const keys = path.split('.');
   ...
   * 获取配置覆盖信息
   */
  getOverrides(): Record<string, OverrideEntry> {
   ...
   * 加载开发环境配置覆盖
   */
  loadDevOverrides(): void {
    try {
      const stored = storageService.getItem('dev-cache-config');
      if (stored) {
        const overrides = JSON.parse(stored as string) as Record<string, OverrideEntry>;
   ...
   */
  createPreset(name: string, config: Record<string, any>) {
  ...
  applyPreset(name: string) {
    ...
    // 应用预设配置
    Object.entries(preset.config as Record<string, any>).forEach(([section, sectionConfig]) => {
   ...
   * 获取所有配置预设
   */
  getPresets(): Record<string, { config: any; createdAt: string }> {
    try {
      const stored = storageService.getItem('cache-config-presets');
      return stored ? (JSON.parse(stored as string) as Record<string, { config: any; createdAt: string }>) : {};
    } catch (error) {
   ...
   */
  importConfig(configData: { current?: any; overrides?: Record<string, OverrideEntry>; presets?: any }) {
    try {
  ...
  getStats() {
    return {
      ...
      lastModified: Math.max(...Object.values(this.overrides).map(o => (o as OverrideEntry).timestamp), 0)
    };
  }
}
...
if (environment.isDevelopment && typeof window !== 'undefined') {
  ...
  window.autocompleteConfig = {
    get: (path: string, defaultValue?: any) => configManager.get(path, defaultValue),
    set: (path: string, value: any, persist: boolean = true) => configManager.set(path, value, persist),
    reset: (path?: string) => configManager.reset(path ?? null),
    export: () => configManager.exportConfig(),
    import: (data: any) => configManager.importConfig(data),
    presets: {
      create: (name: string, config: Record<string, any>) => configManager.createPreset(name, config),
      apply: (name: string) => configManager.applyPreset(name),
      list: () => configManager.getPresets(),
      delete: (name: string) => configManager.deletePreset(name)
    },
