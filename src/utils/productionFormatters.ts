...
type NetworkSpeedOptions = BytesOptions;
...
type FormatterConfig = {
  percentage: PercentageOptions;
  bytes: BytesOptions;
  networkSpeed: NetworkSpeedOptions;
  time: TimeOptions;
  number: NumberOptions;
};

const FORMAT_CONFIG: FormatterConfig = {
  // 百分比格式化
  percentage: {
    decimals: 1,
...
export class ProductionFormatter {
  ...
  constructor(config: Partial<FormatterConfig> = {}) {
    this.config = { ...FORMAT_CONFIG, ...config } as FormatterConfig;
    this.locale = 'zh-CN';
   ...
   */
  formatPercentage(value: number | null | undefined, options: Partial<PercentageOptions> = {}): string {
    const opts: PercentageOptions = { ...this.config.percentage, ...options };
   ...
   */
  formatBytes(bytes: number | null | undefined, options: Partial<BytesOptions> = {}): string {
    const opts: BytesOptions = { ...this.config.bytes, ...options };
   ...
   */
  formatNetworkSpeed(bytesPerSecond: number | null | undefined, options: Partial<NetworkSpeedOptions> = {}): string {
    const opts: NetworkSpeedOptions = { ...this.config.networkSpeed, ...options };
   ...
   */
  formatNumber(value: number | null | undefined, options: Partial<NumberOptions> = {}): string {
    const opts: NumberOptions = { ...this.config.number, ...options };
   ...
   */
  formatTimestamp(timestamp: number | null | undefined, options: Partial<TimeOptions> = {}): string {
    const opts: TimeOptions = { ...this.config.time, ...options };
    ...
    if (opts.showSeconds) {
      (formatOptions as any).second = '2-digit';
    }
   ...
   */
  formatLoadAverage(
    loadAverage: { load1?: number; load5?: number; load15?: number } | null | undefined,
    cpuCores: number = 1
  ): { formatted: string; status: 'critical' | 'warning' | 'moderate' | 'normal'; normalized: string } | string {
    ...
    const getLoadStatus = (load: number): 'critical' | 'warning' | 'moderate' | 'normal' => {
  ...
  formatStatus(status: string): { text: string; color: string; icon: string } {
    ...
    return (statusMap as Record<string, { text: string; color: string; icon: string }>)[status] || statusMap.unknown;
  }
   ...
   */
  smartFormat(value: any, type: string): string {
    switch (type) {
      case 'percentage':
  ...
  updateConfig(newConfig: Partial<FormatterConfig>): void {
    this.config = { ...this.config, ...newConfig } as FormatterConfig;
  }
}
...
// 便捷方法导出
export const formatPercentage = (value: number | null | undefined, options?: Partial<PercentageOptions>) =>
  productionFormatter.formatPercentage(value, options);
export const formatBytes = (bytes: number | null | undefined, options?: Partial<BytesOptions>) =>
  productionFormatter.formatBytes(bytes, options);
export const formatNetworkSpeed = (speed: number | null | undefined, options?: Partial<NetworkSpeedOptions>) =>
  productionFormatter.formatNetworkSpeed(speed, options);
export const formatNumber = (value: number | null | undefined, options?: Partial<NumberOptions>) =>
  productionFormatter.formatNumber(value, options);
export const formatTimestamp = (timestamp: number | null | undefined, options?: Partial<TimeOptions>) =>
  productionFormatter.formatTimestamp(timestamp, options);
export const formatUptime = (seconds: number | null | undefined) => productionFormatter.formatUptime(seconds);
export const formatLoadAverage = (
  loadAverage: { load1?: number; load5?: number; load15?: number } | null | undefined,
  cpuCores?: number
) => productionFormatter.formatLoadAverage(loadAverage, cpuCores);
export const formatStatus = (status: string) => productionFormatter.formatStatus(status);
