...
function normalizeBg(bg) {
  ...
  out.url = typeof out.url === 'string' ? out.url.trim() : '';
  const op = Number(out.opacity);
  out.opacity = Number.isFinite(op) ? Math.min(1, Math.max(0, op)) : 0.5;
  const mode = String(out.mode || 'cover').toLowerCase();
  out.mode = ['cover', 'contain', 'fill', 'none', 'repeat'].includes(mode) ? mode : 'cover';
  return out;
}
 ...
 */
export function applyTerminalBackgroundCss(bg) {
...
/** 清除背景相关 CSS 变量 */
export function clearTerminalBackgroundCss() {
  const root = document.documentElement;
