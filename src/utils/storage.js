...
export function saveToStorage(key, value, options = {}) {
  try {
    ...
    const finalKey = usePrefix ? getPrefixedKey(key, prefix) : key;
    const now = Date.now();
    const storageItem = { value, timestamp: now, expiration: now + expiration };
...
export function cleanupExpiredItems() {
  try {
    ...
    for (const [k, it] of memoryCache.entries()) {
      if (it.expiration < now) memoryCache.delete(k);
    }
...
function startCleanupTimer() {
  ...
  cleanupTimer = setInterval(() => cleanupExpiredItems(), CACHE_CONFIG.cleanupInterval);
}
...
export function createPrefixedStorage(prefix) {
  return {
    get: (k, def) => getFromStorage(k, def, { prefix }),
    set: (k, v, o = {}) => saveToStorage(k, v, { ...o, prefix }),
    remove: k => removeFromStorage(k, { prefix }),
    clear: () => clearStorage({ onlyPrefixed: true, prefix }),
    has: k => hasValidKey(k, { prefix }),
