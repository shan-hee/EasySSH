...
export class AIErrorHandler {
   ...
   */
  handleError(error: any, context: any = {}, severity: string = ErrorSeverity.MEDIUM) {
   ...
   */
  normalizeError(error: any, context: any, severity: string) {
   ...
   * @param {Object} errorInfo 错误信息
   */
  handleCriticalError(errorInfo: any) {
    ElNotification({
      title: '严重错误',
   ...
   * @param {Object} errorInfo 错误信息
   */
  handleHighSeverityError(errorInfo: any) {
    ElNotification({
      title: '错误',
   ...
   * @param {Object} errorInfo 错误信息
   */
  handleMediumSeverityError(errorInfo: any) {
    ElMessage({
      message: this.getUserFriendlyMessage(errorInfo),
      type: 'error',
      duration: 5000
   ...
   * @param {Object} errorInfo 错误信息
   */
  handleLowSeverityError(errorInfo: any) {
    ElMessage({
      message: this.getUserFriendlyMessage(errorInfo),
      type: 'warning',
      duration: 3000
   ...
   */
  getUserFriendlyMessage(errorInfo: any) {
 ...
 */
export function withErrorHandling<T extends any[], R>(fn: (...args: T) => Promise<R> | R, context: any = {}, severity: string = ErrorSeverity.MEDIUM) {
 ...
 * @returns {*} 函数结果或回退值
 */
export function safeExecute<R>(fn: () => R, fallback: R | null = null, context: any = {}) {
  try {
    return fn();
  } catch (error) {
