...
class UserScript {
    ...
    static async searchUserScripts(userId, query, options = {}) {
        ...
        LIMIT ? OFFSET ?
      `);
            const rows = stmt.all(userId, searchPattern, searchPattern, searchPattern, searchPattern, searchPattern, searchPattern, searchPattern, searchPattern, limit, offset);
            return rows.map(row => new UserScript(row));
        }
        catch (error) {
