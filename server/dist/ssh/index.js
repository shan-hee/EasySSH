...
// 导入统一二进制协议
const { BINARY_MSG_TYPE, BinaryMessageDecoder, BinaryMessageSender } = require('./binary-protocol');
// 导入监控处理模块
        ...
        ws.on('message', async (message, isBinary) => {
            try {
                ...
                try {
                    ...
                    const errorResponse = createErrorResponse(ERROR_CODES.INVALID_MESSAGE_FORMAT, 'JSON格式错误');
                ...
                if (!validation.isValid) {
                    ...
                    const errorResponse = createErrorResponse(validation.errorCode, validation.errorMessage, msg.requestId);
                    ws.send(JSON.stringify(errorResponse));
                    return;
                }
                // 使用验证后的消息
