...
// 导入工具模块
const { BINARY_MSG_TYPE, sendBinaryMessage, sendBinarySftpSuccess, sendBinarySftpError, validateSftpSession, safeExec, BinaryMessageDecoder, ChunkedTransfer, ChunkReassembler, ChecksumValidator } = require('./utils');
// 导入SFTP操作处理器
    ...
    try {
        ...
        switch (messageType) {
            ...
            default:
                ...
                sendBinarySftpError(ws, headerData.sessionId, headerData.operationId, `未知的消息类型: ${messageType}`, 'INVALID_MESSAGE_TYPE');
        }
    }
    catch (error) {
        ...
        try {
            ...
            sendBinarySftpError(ws, partialMessage.headerData.sessionId, partialMessage.headerData.operationId, error.message, 'MESSAGE_PROCESSING_ERROR');
        }
        catch (decodeError) {
...
async function handleBinaryUpload(ws, headerData, payloadData, sshSessions) {
    ...
    await safeExec(async () => {
        ...
        if (canceledOperations.has(operationId)) {
            try {
                chunkReassembler.discard(operationId);
            }
            catch (_) { }
...
async function handleBinaryDownloadFolder(ws, headerData, sshSessions) {
    ...
    await safeExec(async () => {
        ...
        if (preflight.code !== 0 || preflight.stdout.includes('__NO_TAR__')) {
            ...
            const sftpSession = sftpSessions.get(sessionId);
            if (!sftpSession)
                throw new Error('SFTP会话不存在，无法回退ZIP');
        ...
        if (!succeeded) {
            ...
            const sftpSession = sftpSessions.get(sessionId);
            if (!sftpSession)
                throw new Error(`远端tar失败且无SFTP会话可用: ${lastErr ? lastErr.message : 'unknown error'}`);
    ...
    try {
        ...
        if (activeTransfers.has(operationId)) {
            ...
            if (entry.type === 'file' && entry.stream) {
                ...
                try {
                    if (typeof entry.stream.signal === 'function')
                        entry.stream.signal('TERM');
                }
                catch (e) { }
                try {
                    entry.stream.close();
                ...
                try {
                    if (typeof entry.archive.abort === 'function')
                        entry.archive.abort();
                }
                catch (e) { }
                try {
                    if (typeof entry.archive.destroy === 'function')
                        entry.archive.destroy();
                }
                catch (e) { }
            }
