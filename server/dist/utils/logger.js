...
const maybeDist = node_path_1.default.resolve(__dirname, '..'); // e.g. server/dist/utils -> server/dist
const BASE_DIR = node_path_1.default.basename(maybeDist) === 'dist' ? node_path_1.default.resolve(maybeDist, '..') : maybeDist; // server
const IN_DOCKER = (() => { try {
...
const DEFAULT_LOG_DIR = IN_DOCKER ? '/easyssh/logs' : node_path_1.default.join(BASE_DIR, 'logs');
// 日志配置
...
function ensureLogDirectory() {
    try {
        if (!node_fs_1.default.existsSync(LOG_CONFIG.logDirectory)) {
            node_fs_1.default.mkdirSync(LOG_CONFIG.logDirectory, { recursive: true });
        }
    }
    catch (error) {
...
function checkAndRotateLog(logFilePath) {
    try {
        ...
        const stats = node_fs_1.default.statSync(logFilePath);
        if (stats.size > LOG_CONFIG.maxFileSize) {
            for (let i = LOG_CONFIG.maxBackupFiles - 1; i >= 1; i--) {
                ...
                if (node_fs_1.default.existsSync(oldFile)) {
                    if (i === LOG_CONFIG.maxBackupFiles - 1) {
                        node_fs_1.default.unlinkSync(oldFile);
                    }
                    else {
            ...
            if (node_fs_1.default.existsSync(`${logFilePath}.1`)) {
                node_fs_1.default.unlinkSync(`${logFilePath}.1`);
            }
            node_fs_1.default.renameSync(logFilePath, `${logFilePath}.1`);
        }
    }
    catch (error) {
...
function cleanupOldLogs() {
    try {
        const files = node_fs_1.default.readdirSync(LOG_CONFIG.logDirectory);
        const now = Date.now();
        const maxAge = LOG_CONFIG.maxLogAge * 24 * 60 * 60 * 1000;
        files.forEach(file => {
            if (file.startsWith(LOG_CONFIG.logFileName)) {
                const filePath = node_path_1.default.join(LOG_CONFIG.logDirectory, file);
...
function writeToFile(logMessage) {
    ...
    try {
        ensureLogDirectory();
        const logFilePath = node_path_1.default.join(LOG_CONFIG.logDirectory, LOG_CONFIG.logFileName);
        checkAndRotateLog(logFilePath);
        node_fs_1.default.appendFile(logFilePath, `${logMessage}\n`, (error) => {
