...
const updateUserScript = async (req, res) => {
    try {
        ...
        // 更新脚本信息
        if (name !== undefined)
            userScript.name = name.trim();
        if (description !== undefined)
            userScript.description = description.trim();
        if (command !== undefined)
            userScript.command = command.trim();
        if (Array.isArray(tags))
            userScript.tags = tags;
        if (Array.isArray(keywords))
            userScript.keywords = keywords;
        if (category !== undefined)
            userScript.category = category.trim() || '我的脚本';
        await userScript.save();
        res.json({
            success: true,
            script: userScript.toJSON(),
...
const recordScriptUsage = async (req, res) => {
    ...
    `);
        insertHistoryStmt.run(userId, scriptId || null, userScriptId || null, scriptName, command, new Date().toISOString());
        // 更新使用次数
        if (scriptId) {
...
const executeScript = async (req, res) => {
    try {
        const userId = req.user.id;
        const { scriptId, scriptName, command, serverId, serverName, host, port, username } = req.body;
        if (!command) {
            return res.status(400).json({
                success: false,
                message: '脚本命令不能为空'
    ...
    `);
        insertExecutionStmt.run(userId, validScriptId, scriptName, command, connection.id, // 关联连接ID，便于追溯
        serverName, host, port || 22, username, executionResult.stdout, executionResult.stderr, executionResult.exitCode, executionResult.executedAt);
        res.json({
            success: true,
...
const updateUserFavorites = async (req, res) => {
    try {
        ...
        try {
            // 清除现有收藏
            db.prepare('DELETE FROM user_script_favorites WHERE user_id = ?').run(userId);
            // 添加新收藏
            const insertStmt = db.prepare('INSERT INTO user_script_favorites (user_id, script_id, created_at) VALUES (?, ?, ?)');
...
const toggleScriptFavorite = async (req, res) => {
    try {
        ...
        if (existing) {
            // 取消收藏
            db.prepare('DELETE FROM user_script_favorites WHERE user_id = ? AND script_id = ?').run(userId, scriptId);
            res.json({
                success: true,
                isFavorite: false,
                message: '已取消收藏'
            ...
            // 添加收藏
            db.prepare('INSERT INTO user_script_favorites (user_id, script_id, created_at) VALUES (?, ?, ?)').run(userId, scriptId, new Date().toISOString());
            res.json({
                success: true,
                isFavorite: true,
                message: '已添加到收藏'
