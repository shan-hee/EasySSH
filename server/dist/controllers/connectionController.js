...
const { validateConnection } = validators_1.default;
const { processConnectionSensitiveData, decryptPassword, decryptPrivateKey } = encryption_1.default;
const getNextSortOrderForUser = (userId) => {
    try {
        ...
        logger_1.default.warn('计算下一个连接排序值失败，使用默认值1', error);
        return 1;
    }
};
/**
...
const getUserConnections = async (req, res) => {
    try {
        ...
        // 从数据库获取用户连接
        const connections = db.prepare('SELECT * FROM connections WHERE user_id = ? ORDER BY sort_order ASC, created_at ASC').all(userId);
        ...
        const formattedConnections = connections.map((conn) => {
            try {
                ...
                logger_1.default.error(`处理连接数据错误：${err.message}`, { connectionId: conn.id });
                return null;
            }
        ...
        logger_1.default.error('获取用户连接失败', error);
        res.status(500).json({
            success: false,
            message: '获取连接列表失败',
            error: error.message
...
const addConnection = async (req, res) => {
    try {
        ...
        // 检查是否已存在相同的连接（防重复）
        const existingConnection = db.prepare('SELECT id FROM connections WHERE host = ? AND port = ? AND username = ? AND user_id = ?').get(connection.host, connection.port || 22, connection.username, userId);
        ...
        const sortOrder = typeof connection.sortOrder === 'number' ? connection.sortOrder : getNextSortOrderForUser(userId);
        // 开始事务
      ...
      ) VALUES (?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?)`).run(connectionId, userId, encryptedConnection.name || `${encryptedConnection.username}@${encryptedConnection.host}`, encryptedConnection.host, encryptedConnection.port || 22, encryptedConnection.username, encryptedConnection.rememberPassword ? encryptedConnection.password : '', encryptedConnection.rememberPassword ? 1 : 0, encryptedConnection.privateKey || '', encryptedConnection.passphrase || '', encryptedConnection.authType || 'password', encryptedConnection.description || '', encryptedConnection.group || '默认分组', JSON.stringify(encryptedConnection.config || {}), sortOrder, now, now);
            // 提交事务
        ...
        logger_1.default.error('添加连接失败', error);
        res.status(500).json({
            success: false,
            message: '添加连接失败',
            error: error.message
...
const updateConnection = async (req, res) => {
    try {
        ...
        // 检查连接是否存在并属于当前用户
        const existingConnection = db.prepare('SELECT id, sort_order FROM connections WHERE id = ? AND user_id = ?').get(connectionId, userId);
        if (!existingConnection) {
        ...
        // 加密敏感数据
        const encryptedConnection = processConnectionSensitiveData(connection, true);
        const sortOrder = typeof connection.sortOrder === 'number'
        ...
        sort_order = ?,
        updated_at = ?
      WHERE id = ? AND user_id = ?`).run(encryptedConnection.name || `${encryptedConnection.username}@${encryptedConnection.host}`, encryptedConnection.host, encryptedConnection.port || 22, encryptedConnection.username, encryptedConnection.rememberPassword ? encryptedConnection.password : '', encryptedConnection.rememberPassword ? 1 : 0, encryptedConnection.privateKey || '', encryptedConnection.passphrase || '', encryptedConnection.authType || 'password', encryptedConnection.description || '', encryptedConnection.group || '默认分组', JSON.stringify(connection.config || {}), sortOrder, now, connectionId, userId);
        res.json({
            success: true,
            message: '连接更新成功'
        });
        ...
        logger_1.default.error('更新连接失败', error);
        res.status(500).json({
            success: false,
            message: '更新连接失败',
            error: error.message
...
const deleteConnection = async (req, res) => {
    try {
        ...
        const connectionId = req.params.id;
        // 检查连接是否存在并属于当前用户
        const existingConnection = db.prepare('SELECT id FROM connections WHERE id = ? AND user_id = ?').get(connectionId, userId);
        if (!existingConnection) {
            return res.status(404).json({
                success: false,
        ...
        // 删除连接
        db.prepare('DELETE FROM connections WHERE id = ? AND user_id = ?').run(connectionId, userId);
        // 同时从收藏和历史记录中删除
        db.prepare('DELETE FROM connection_favorites WHERE connection_id = ? AND user_id = ?').run(connectionId, userId);
        db.prepare('DELETE FROM connection_history WHERE connection_id = ? AND user_id = ?').run(connectionId, userId);
        db.prepare('DELETE FROM connection_pinned WHERE connection_id = ? AND user_id = ?').run(connectionId, userId);
        res.json({
            success: true,
            message: '连接删除成功'
        });
        ...
        logger_1.default.error('删除连接失败', error);
        res.status(500).json({
            success: false,
            message: '删除连接失败',
            error: error.message
...
const getFavorites = async (req, res) => {
    try {
        const userId = req.user.id;
        // 获取收藏连接ID列表
        const favorites = db.prepare('SELECT connection_id FROM connection_favorites WHERE user_id = ?').all(userId);
        ...
        logger_1.default.error('获取收藏连接失败', error);
        res.status(500).json({
            success: false,
            message: '获取收藏连接失败',
            error: error.message
...
const updateFavorites = async (req, res) => {
    try {
        ...
        try {
            ...
            // 添加新收藏
            const insertStmt = db.prepare('INSERT INTO connection_favorites (user_id, connection_id) VALUES (?, ?)');
        ...
        logger_1.default.error('更新收藏连接失败', error);
        res.status(500).json({
            success: false,
            message: '更新收藏连接失败',
            error: error.message
...
const getHistory = async (req, res) => {
    try {
        ...
        const history = db.prepare(`SELECT id AS entry_id, connection_id, name, host, port, username, description, group_name, auth_type, timestamp
       ...
       LIMIT 20`).all(userId);
        ...
        logger_1.default.error('获取历史记录失败', error);
        res.status(500).json({
            success: false,
            message: '获取历史记录失败',
            error: error.message
...
const addToHistory = async (req, res) => {
    try {
        ...
        try {
            ...
            // 无条件插入历史记录
            const insertInfo = db.prepare(`INSERT INTO connection_history
         (user_id, connection_id, name, host, port, username, description, group_name, auth_type, timestamp)
         VALUES (?, ?, ?, ?, ?, ?, ?, ?, ?, ?)`).run(userId, connection.id, name, connection.host, port, connection.username, description, groupName, authType, timestamp);
            // 限制历史记录数量为20条
            db.prepare(`DELETE FROM connection_history
         WHERE user_id = ? AND id NOT IN (
           ...
           LIMIT 20
            ...
            // 提交事务
            db.prepare('COMMIT').run();
            const entryId = insertInfo && typeof insertInfo.lastInsertRowid !== 'undefined'
        ...
        logger_1.default.error('添加历史记录失败', error);
        res.status(500).json({
            success: false,
            message: '添加到历史记录失败',
            error: error.message
...
const removeHistoryEntry = async (req, res) => {
    try {
        ...
        logger_1.default.error('按条删除历史记录失败', error);
        res.status(500).json({
            success: false,
            message: '按条删除历史记录失败',
            error: error.message
...
const clearHistory = async (req, res) => {
    try {
        ...
        logger_1.default.error('清空历史记录失败', error);
        res.status(500).json({
            success: false,
            message: '清空历史记录失败',
            error: error.message
...
const getPinned = async (req, res) => {
    try {
        const userId = req.user.id;
        // 获取置顶连接（含时间戳）
        const pinned = db.prepare('SELECT connection_id, pinned_at FROM connection_pinned WHERE user_id = ?').all(userId);
        ...
        logger_1.default.error('获取置顶连接失败', error);
        res.status(500).json({
            success: false,
            message: '获取置顶连接失败',
            error: error.message
...
const getOverview = async (req, res) => {
    try {
        ...
        const rows = db.prepare('SELECT * FROM connections WHERE user_id = ? ORDER BY sort_order ASC, created_at ASC').all(userId);
        const connections = rows
            ...
            try {
                ...
                logger_1.default.error(`处理连接数据错误：${e.message}`, { connectionId: conn.id });
                return null;
            }
        })
            .filter(Boolean);
        ...
        // 3) 历史记录（最近20条）
        const histRows = db
            .prepare(`SELECT id AS entry_id, connection_id, name, host, port, username, description, group_name, auth_type, timestamp
         FROM connection_history
         WHERE user_id = ?
         ORDER BY timestamp DESC
         LIMIT 20`)
        ...
        logger_1.default.error('获取连接汇总数据失败', error);
...
const updatePinned = async (req, res) => {
    try {
        ...
        try {
            ...
            // 添加新置顶（带时间戳）
            const insertStmt = db.prepare('INSERT INTO connection_pinned (user_id, connection_id, pinned_at) VALUES (?, ?, ?)');
        ...
        logger_1.default.error('更新置顶连接失败', error);
        res.status(500).json({
            success: false,
            message: '更新置顶连接失败',
            error: error.message
...
const updateConnectionOrder = async (req, res) => {
    try {
        ...
        const updateStmt = db.prepare('UPDATE connections SET sort_order = ?, updated_at = ? WHERE id = ? AND user_id = ?');
        const now = new Date().toISOString();
        db.prepare('BEGIN TRANSACTION').run();
        try {
        ...
        logger_1.default.error('更新连接排序失败', error);
...
const syncConnections = async (req, res) => {
    try {
        ...
        try {
            // 同步连接
            // 首先获取现有连接ID
            const existingConnections = db.prepare('SELECT id FROM connections WHERE user_id = ?').all(userId);
            const existingIds = new Set(existingConnections.map((conn) => conn.id));
            const now = new Date().toISOString();
            // 准备语句
            const updateStmt = db.prepare(`UPDATE connections SET
          name = ?,
          host = ?,
          port = ?,
          username = ?,
        ...
        WHERE id = ? AND user_id = ?`);
            const insertStmt = db.prepare(`INSERT INTO connections (
          id, user_id, name, host, port, username, password, 
          ...
          description, group_name, config, sort_order, created_at, updated_at
            ...
            for (let index = 0; index < connections.length; index += 1) {
                ...
                // 加密敏感数据
                const encryptedConnection = processConnectionSensitiveData(connection, true);
                const sortOrder = typeof connection.sortOrder === 'number' ? connection.sortOrder : index + 1;
                if (existingIds.has(connection.id)) {
                    // 更新现有连接
                    updateStmt.run(encryptedConnection.name || `${encryptedConnection.username}@${encryptedConnection.host}`, encryptedConnection.host, encryptedConnection.port || 22, encryptedConnection.username, encryptedConnection.rememberPassword ? encryptedConnection.password : '', encryptedConnection.rememberPassword ? 1 : 0, encryptedConnection.privateKey || '', encryptedConnection.passphrase || '', encryptedConnection.authType || 'password', encryptedConnection.description || '', encryptedConnection.group || '默认分组', JSON.stringify(encryptedConnection.config || {}), sortOrder, now, connection.id, userId);
                }
                else {
                    // 添加新连接
                    insertStmt.run(connection.id, userId, encryptedConnection.name || `${encryptedConnection.username}@${encryptedConnection.host}`, encryptedConnection.host, encryptedConnection.port || 22, encryptedConnection.username, encryptedConnection.rememberPassword ? encryptedConnection.password : '', encryptedConnection.rememberPassword ? 1 : 0, encryptedConnection.privateKey || '', encryptedConnection.passphrase || '', encryptedConnection.authType || 'password', encryptedConnection.description || '', encryptedConnection.group || '默认分组', JSON.stringify(encryptedConnection.config || {}), sortOrder, now, now);
                }
            }
            // 同步收藏
            db.prepare('DELETE FROM connection_favorites WHERE user_id = ?').run(userId);
            const insertFavoriteStmt = db.prepare('INSERT INTO connection_favorites (user_id, connection_id) VALUES (?, ?)');
            for (const connectionId of favorites) {
                insertFavoriteStmt.run(userId, connectionId);
            }
            // 同步历史记录
            db.prepare('DELETE FROM connection_history WHERE user_id = ?').run(userId);
            const insertHistoryStmt = db.prepare(`INSERT INTO connection_history
         (user_id, connection_id, name, host, port, username, description, group_name, auth_type, timestamp)
         VALUES (?, ?, ?, ?, ?, ?, ?, ?, ?, ?)`);
            for (const historyItem of history) {
                ...
                if (connectionInfo) {
                    ...
                    insertHistoryStmt.run(userId, historyItem.id, name, connectionInfo.host, port, connectionInfo.username, description, groupName, authType, historyItem.timestamp);
                }
            }
            // 同步置顶（带时间戳）
            db.prepare('DELETE FROM connection_pinned WHERE user_id = ?').run(userId);
            const insertPinnedStmt = db.prepare('INSERT INTO connection_pinned (user_id, connection_id, pinned_at) VALUES (?, ?, ?)');
            for (const connectionId in pinned) {
                if (pinned[connectionId]) {
                    const pinnedAt = typeof pinned[connectionId] === 'number' ? pinned[connectionId] : Date.now();
        ...
        logger_1.default.error('同步连接数据失败', error);
        res.status(500).json({
            success: false,
            message: '同步连接数据失败',
            error: error.message
