...
export class KeyboardManager extends EventEmitter {
  ...
  _initDefaultShortcuts() {
    ...
    this.registerShortcut({
      ...
      action: 'help.show',
      description: '显示帮助',
      handler: () => this.emit('action', 'help.show')
    });
    ...
    this.registerShortcut({
      scope: KeyScope.GLOBAL,
      key: `${ModifierKeys.CTRL}+n`,
      action: 'connection.new',
      description: '新建连接',
      handler: () => this.emit('action', 'connection.new')
    });
    
    this.registerShortcut({
      scope: KeyScope.GLOBAL,
      key: `${ModifierKeys.CTRL}+o`,
      action: 'connection.open',
      description: '打开连接',
      handler: () => this.emit('action', 'connection.open')
    });
    
    this.registerShortcut({
      scope: KeyScope.GLOBAL,
      key: `${ModifierKeys.CTRL}+Tab`,
      action: 'tab.next',
      description: '下一个标签页',
      handler: () => this.emit('action', 'tab.next')
    });
    
    this.registerShortcut({
      scope: KeyScope.GLOBAL,
      key: `${ModifierKeys.CTRL}+${ModifierKeys.SHIFT}+Tab`,
      action: 'tab.previous',
      description: '上一个标签页',
      handler: () => this.emit('action', 'tab.previous')
    });
    
    // 应用范围的快捷键
    this.registerShortcut({
      scope: KeyScope.APP,
      key: `${ModifierKeys.CTRL}+s`,
      action: 'session.save',
      description: '保存会话',
      handler: () => this.emit('action', 'session.save')
    });
    
    this.registerShortcut({
      scope: KeyScope.APP,
      key: `${ModifierKeys.ALT}+1`,
      action: 'sidebar.toggle',
      description: '切换侧边栏',
      handler: () => this.emit('action', 'sidebar.toggle')
    });
    
    // 编辑器范围的快捷键
    this.registerShortcut({
      scope: KeyScope.EDITOR,
      key: `${ModifierKeys.CTRL}+c`,
      action: 'editor.copy',
      description: '复制',
      handler: () => this.emit('action', 'editor.copy')
    });
    
    this.registerShortcut({
      scope: KeyScope.EDITOR,
      key: `${ModifierKeys.CTRL}+v`,
      action: 'editor.paste',
      description: '粘贴',
      handler: () => this.emit('action', 'editor.paste')
    });
    ...
    this.registerShortcut({
      scope: KeyScope.GLOBAL,
      key: `${ModifierKeys.CTRL}+${ModifierKeys.ALT}+c`,
      action: 'accessibility.toggleHighContrast',
      description: '切换高对比度模式',
      handler: () => accessibilityService.setHighContrast(!accessibilityService.config.highContrast)
    });
  }
